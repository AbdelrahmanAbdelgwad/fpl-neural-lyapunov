...
import neural_network_lyapunov.monotonic_lyapunov_init.custom_mip_utils as mip_utils
...
import neural_network_lyapunov.monotonic_lyapunov_init.custom_relu_to_optimization_utils as relu_to_optimization_utils
...
def relu_activation_binary_to_pattern(relu_network, activation_binary):
    ...
    assert np.all(np.logical_or(activation_binary == 1, activation_binary == 0))
    assert isinstance(relu_network[-1], monotonic_utils.LinearyLayer)
    num_relu_layers = int((len(relu_network) - 1) / 2)
    total_relu_units = np.sum(
        np.array([relu_network[2 * i].out_features for i in range(num_relu_layers)])
    )
    ...
    for i in range(num_relu_layers):
        ...
        activation_pattern[i] = list(
            activation_binary[relu_unit_count : relu_unit_count + num_relu_in_layer]
            == 1
        )
...
def set_activation_warmstart(relu, beta, x_warmstart):
    ...
    for layer in activation:
        for unit in layer:
            if unit:
                beta[unit_counter].start = 1.0
            else:
                beta[unit_counter].start = 0.0
            unit_counter += 1
    ...
    for layer in model_relu:
        if isinstance(layer, monotonic_utils.LinearyLayer):
            ...
            b_layer = layer.weight @ b_layer + (
                ...
                else torch.zeros((layer.weight.shape[0], 1), dtype=dtype)
            )
            num_linear_layer_output = layer.weight.shape[0]
        elif isinstance(layer, nn.ReLU) or isinstance(layer, nn.LeakyReLU):
            assert len(activation_pattern[relu_layer_count]) == num_linear_layer_output
            for row, activation_flag in enumerate(activation_pattern[relu_layer_count]):
                if activation_flag:
            ...
            raise Exception("The ReLU network only allows ReLU and linear units.")
    g = A_layer.T
    h = b_layer
    return (g, h, P, q)


class ReLUMixedIntegerConstraintsReturn(gurobi_torch_mip.MixedIntegerConstraintsReturn):
    def __init__(self):
...
class ReLUGradientTimesVecMixedIntegerConstraintsReturn(
    ...
    def __init__(self):
        super(ReLUGradientTimesVecMixedIntegerConstraintsReturn, self).__init__()
...
class ReLUFreePattern:
    ...
    def strengthen_mip_at_point(
        self, pt: tuple, linear_inputs_lo: torch.Tensor, linear_inputs_up: torch.Tensor
    ):
        """
        ...
        for relu_layer_count in range(len(self.relu_unit_index)):
            ...
            assert (
                len(self.relu_unit_index[relu_layer_count]) == linear_layer.out_features
            )
            linear_input_indices = np.arange(
                linear_input_count, linear_input_count + linear_layer.in_features
            ).tolist()
            ...
            for j in range(len(self.relu_unit_index[relu_layer_count])):
                beta_hat = relu_activations[self.relu_unit_index[relu_layer_count][j]]
                w = linear_layer.weight.data[j]
                b = (
                    linear_layer.bias.data[j]
                    if linear_layer.bias is not None
                    else torch.tensor(0, dtype=self.dtype)
                )
                index_set = mip_utils.find_index_set_to_strengthen(
                    w, linear_input_lo, linear_input_up, linear_input, beta_hat
                )
                ...
                if (
                    relu_output
                    > x_coeff @ linear_input
                    + binary_coeff * beta_hat
                    + constant
                    + 1e-12
                ):
                    ...
                    # <= constant
                    slack_coeff = torch.zeros((self.num_relu_units,), dtype=self.dtype)
                    slack_coeff[self.relu_unit_index[relu_layer_count][j]] = 1.0
                    if relu_layer_count == 0:
                        ...
                        Ain_input.append(
                            torch.zeros(
                                (1, self.model[0].in_features), dtype=self.dtype
                            )
                        )
                        slack_coeff[self.relu_unit_index[relu_layer_count - 1]] = (
                            -x_coeff
                        )
                    Ain_slack.append(slack_coeff.reshape((1, -1)))
                    binary_all_coeff = torch.zeros(
                        (1, self.num_relu_units), dtype=self.dtype
                    )
                    binary_all_coeff[0, self.relu_unit_index[relu_layer_count][j]] = (
                        -binary_coeff
                    )
        ...
        return (
            torch.cat(Ain_input, dim=0),
            torch.cat(Ain_slack, dim=0),
            torch.cat(Ain_binary, dim=0),
            torch.stack(rhs_in),
        )

    def strengthen_relu_mip_at_solution(
        self,
        prog: gurobi_torch_mip.GurobiTorchMIP,
        x_var: list,
        slack_var: list,
        binary_var: list,
        ...
        assert isinstance(relu_mip_cnstr_return, ReLUMixedIntegerConstraintsReturn)
        linear_inputs = torch.tensor(
            [v.x for v in x_var] + [v.x for v in slack_var], dtype=prog.dtype
        )
        relu_activations = torch.tensor([v.x for v in binary_var], dtype=prog.dtype)
        linear_inputs_lo = torch.cat(
            (relu_mip_cnstr_return.nn_input_lo, relu_mip_cnstr_return.relu_output_lo)
        )
        linear_inputs_up = torch.cat(
            (relu_mip_cnstr_return.nn_input_up, relu_mip_cnstr_return.relu_output_up)
        )
        Ain_x, Ain_slack, Ain_binary, rhs_in = self.strengthen_mip_at_point(
            (linear_inputs, relu_activations), linear_inputs_lo, linear_inputs_up
        )
        if Ain_x is not None:
            prog.addMConstr(
                [Ain_x, Ain_slack, Ain_binary],
    ...
    def _compute_linear_output_bound_by_optimization(
        self,
        layer_index,
        linear_output_row_index,
        previous_neuron_input_lo: np.ndarray,
        previous_neuron_input_up: np.ndarray,
        network_input_lo: np.ndarray,
        ...
        bij = (
            self.model[2 * layer_index].bias[linear_output_row_index]
            if self.model[2 * layer_index].bias is not None
            else torch.tensor(0, dtype=self.dtype)
        )
        prog.setObjective([Wij], [z_curr], constant=bij, sense=gurobipy.GRB.MAXIMIZE)
        ...
        if prog.gurobi_model.status == gurobipy.GRB.Status.OPTIMAL:
            ...
            up_input_val = torch.tensor([v.x for v in network_input], dtype=self.dtype)
        ...
        prog.setObjective([Wij], [z_curr], constant=bij, sense=gurobipy.GRB.MINIMIZE)
        prog.gurobi_model.optimize()
        # The value of the network input that gives the lower bound.
        lo_input_val = None
        if prog.gurobi_model.status == gurobipy.GRB.Status.OPTIMAL:
            linear_output_lo = prog.gurobi_model.ObjVal
            lo_input_val = torch.tensor([v.x for v in network_input], dtype=self.dtype)
        elif prog.gurobi_model.status == gurobipy.GRB.Status.UNBOUNDED:
    ...
    def _compute_layer_bound(
        ...
        for layer_count in range(len(self.relu_unit_index)):
            ...
            bias = (
                self.model[2 * layer_count].bias
                if self.model[2 * layer_count].bias is not None
                else torch.zeros(
                    (self.model[2 * layer_count].out_features,), dtype=self.dtype
                )
            )
            if method == mip_utils.PropagateBoundsMethod.IA:
                z_pre_relu_lo[z_indices], z_pre_relu_up[z_indices] = (
                    mip_utils.compute_range_by_IA(
                        self.model[2 * layer_count].weight,
                        bias,
                        linear_layer_input_lo,
                        linear_layer_input_up,
                    )
                )
            else:
                ...
                for j in range(self.model[2 * layer_count].out_features):
                    ...
                    z_pre_relu_lo[neuron_index], z_pre_relu_up[neuron_index], _, _ = (
                ...
                if method == mip_utils.PropagateBoundsMethod.IA_MIP:
                    ...
                    z_pre_relu_lo_ia, z_pre_relu_up_ia = mip_utils.compute_range_by_IA(
                        self.model[2 * layer_count].weight,
                        bias,
                        linear_layer_input_lo,
                        linear_layer_input_up,
                    ...
                    for j in range(self.model[2 * layer_count].out_features):
                        neuron_index = self.relu_unit_index[layer_count][j]
                        if (
                            torch.abs(z_pre_relu_lo[neuron_index] - z_pre_relu_lo_ia[j])
                            < 1e-4
                        ):
                            ...
                            if z_pre_relu_lo[neuron_index] > 0:
                                ...
                                z_pre_relu_lo[neuron_index] += (
                                    0.01
                                    * torch.clamp(z_pre_relu_lo_ia[j], max=0.0).detach()
                                )
                            else:
                                z_pre_relu_lo[neuron_index] *= 0.99
                                z_pre_relu_lo[neuron_index] += (
                                    0.01 * z_pre_relu_lo_ia[j].detach()
                                )
                        if (
                            torch.abs(z_pre_relu_up[neuron_index] - z_pre_relu_up_ia[j])
                            < 1e-4
                        ):
                            ...
                            if z_pre_relu_up[neuron_index] <= 0:
                                ...
                                z_pre_relu_up[neuron_index] += (
                                    0.01
                                    * torch.clamp(z_pre_relu_up_ia[j], max=0.0).detach()
                                )
                            else:
                                z_pre_relu_up[neuron_index] *= 0.99
                                z_pre_relu_up[neuron_index] += (
                                    0.01 * z_pre_relu_up_ia[j].detach()
                                )

            z_post_relu_lo[z_indices], z_post_relu_up[z_indices] = (
                mip_utils.propagate_bounds(
                    self.model[2 * layer_count + 1],
                    z_pre_relu_lo[z_indices],
    ...
    def _compute_network_output_bounds(
        self,
        ...
        if method == mip_utils.PropagateBoundsMethod.IA:
            ...
            binary_var_type = mip_utils.binary_var_type_per_method(method)
            linear_output_lo = torch.empty(
                (self.model[-1].out_features,), dtype=self.dtype
            )
            linear_output_up = torch.empty(
                (self.model[-1].out_features,), dtype=self.dtype
            )
            if method == mip_utils.PropagateBoundsMethod.IA_MIP:
            ...
            for i in range(self.model[-1].out_features):
                linear_output_lo[i], linear_output_up[i], _, _ = (
                    self._compute_linear_output_bound_by_optimization(
                ...
                if method == mip_utils.PropagateBoundsMethod.IA_MIP:
                    if torch.abs(linear_output_lo[i] - linear_output_lo_ia[i]) < 1e-4:
                        linear_output_lo[i] = linear_output_lo_ia[i]
                    else:
                        linear_output_lo[i] *= 0.99
                        linear_output_lo[i] += 0.01 * linear_output_lo_ia[i]
                    if torch.abs(linear_output_up[i] - linear_output_up_ia[i]) < 1e-3:
                        linear_output_up[i] = linear_output_up_ia[i].detach()
                    else:
                        linear_output_up[i] *= 0.99
                        linear_output_up[i] += 0.01 * linear_output_up_ia[i].detach()

            return linear_output_lo, linear_output_up

    def _output_constraint_given_bounds(self, z_pre_relu_lo, z_pre_relu_up, x_lo, x_up):
        ...
        Ain_input = torch.zeros(
            (4 * self.num_relu_units + 2 * self.x_size, self.x_size), dtype=self.dtype
        )
        ...
        rhs_in = torch.empty(
            (4 * self.num_relu_units + 2 * self.x_size,), dtype=self.dtype
        )
        ...
        for layer_count in range(len(self.relu_unit_index)):
            ...
            if Ain_z_curr.shape[0] > 0:
                ...
                rhs_in[ineq_constr_count : ineq_constr_count + Ain_z_curr.shape[0]] = (
                    rhs_in_layer
                )
            ...
            if Aeq_z_curr.shape[0] > 0:
                ...
                rhs_eq[eq_constr_count : eq_constr_count + Aeq_z_curr.shape[0]] = (
                    rhs_eq_layer
                )
        ...
        mip_constr_return.Aout_slack = torch.zeros(
            (self.model[-1].out_features, self.num_relu_units), dtype=self.dtype
        )
        mip_constr_return.Aout_slack[:, self.relu_unit_index[-1]] = self.model[
            -1
        ].weight.clone()
    ...
    def output_constraint(self, x_lo, x_up, method: mip_utils.PropagateBoundsMethod):
        """
    ...
    def compute_relu_unit_outputs_and_activation(self, x):
        ...
        for layer in self.model:
            if isinstance(layer, monotonic_utils.LinearyLayer):
                ...
                z_layer = layer.forward(z_layer)
                z[relu_unit_count : relu_unit_count + z_layer.numel()] = (
                    z_layer.reshape((-1, 1))
                )
                relu_unit_count += z_layer.numel()
            else:
                raise Exception(
                    "compute_relu_unit_outputs_and_activation: "
    ...
    def compute_alpha_index(self, relu_unit_layer_indices):
        ...
        for i in range(len(self.relu_unit_index)):
            ...
            index = index * len(self.relu_unit_index[i]) + relu_unit_layer_indices[i]
        return index
    ...
    def output_gradient(self):
        ...
        for layer in self.model:
            if isinstance(layer, monotonic_utils.LinearyLayer):
                if layer_count == 0:
                    linear_unit_gradient = LinearUnitGradient((), layer.weight.clone())
                    ...
                    while last_layer_linear_unit_gradient_count < queue_size:
                        ...
                        for layer_relu_unit_index in range(
                            len(self.relu_unit_index[layer_count - 1])
                        ):
                            ...
                            linear_unit_gradient = LinearUnitGradient(
                                ...
                                + (layer_relu_unit_index,),
                                layer.weight[:, layer_relu_unit_index].reshape((-1, 1))
                ...
                raise Exception(
                    "output_gradient: currently only "
        ...
        while not layer_linear_unit_gradients.empty():
            ...
            B1[ineq_constraint_count + num_layers][alpha_index] = -1.0
            d[ineq_constraint_count + num_layers][0] = num_layers - 1.0
            for layer_count in range(num_layers):
                B1[ineq_constraint_count + layer_count][alpha_index] = 1.0
                beta_index = self.relu_unit_index[layer_count][
                    layer_linear_unit_gradient.activated_relu_indices[layer_count]
                ]
                d[ineq_constraint_count + layer_count][0] = 0.0
                B2[ineq_constraint_count + layer_count][beta_index] = -1.0
    ...
    def output_gradient_times_vector_w_bounds(
        self, z_lo, z_up, Wz_lo, Wz_up
    ) -> ReLUGradientTimesVecMixedIntegerConstraintsReturn:
        """
        ...
        A_y * y + A_slack * slack + A_beta * β ≤ rhs
        """
        A_y = torch.zeros((4 * self.num_relu_units, self.x_size), dtype=self.dtype)
        A_slack = torch.zeros(
            (4 * self.num_relu_units, self.num_relu_units), dtype=self.dtype
        )
        A_beta = torch.zeros(
            (4 * self.num_relu_units, self.num_relu_units), dtype=self.dtype
        )
        ...
        for layer in self.model:
            if isinstance(layer, monotonic_utils.LinearyLayer):
                ...
                if layer_count == len(self.relu_unit_index):
                    ...
                    A_out[:, self.relu_unit_index[layer_count - 1]] = layer.weight
                ...
                for j in range(len(self.relu_unit_index[layer_count])):
                    ...
                    if layer_count == 0:
                        A_y[ineq_count : ineq_count + 4] = A_pre.reshape((-1, 1)) @ Wi[
                            j
                        ].reshape((1, -1))
                    else:
                        A_slack[
                            ineq_count : ineq_count + 4,
                            self.relu_unit_index[layer_count - 1],
                        ] = A_pre.reshape((-1, 1)) @ Wi[j].reshape((1, -1))
                    A_slack[
                        ineq_count : ineq_count + 4,
                ...
                raise Exception(
                    "output_gradient_times_vector: we currently "
    ...
    def output_gradient_times_vector(
        self, vector_lower, vector_upper
    ) -> ReLUGradientTimesVecMixedIntegerConstraintsReturn:
        """
        We want to compute the gradient of the network ∂ReLU(x)/∂x times a
        vector y: ∂ReLU(x)/∂x  * y, and reformulate this product as
        ...
        return self.output_gradient_times_vector_w_bounds(z_lo, z_up, Wz_lo, Wz_up)

    def _compute_Wz_bounds_optimization(
        self, prog: gurobipy.Model, z0: list, beta: list
    ):
        """
        ...
        for i in range(len(z0)):
            prog.setObjective(gurobipy.LinExpr(1, z0[i]), sense=gurobipy.GRB.MAXIMIZE)
            prog.optimize()
            assert prog.status == gurobipy.GRB.Status.OPTIMAL
            z_up[0][i] = prog.ObjVal
            prog.setObjective(gurobipy.LinExpr(1, z0[i]), sense=gurobipy.GRB.MINIMIZE)
            prog.optimize()
            assert prog.status == gurobipy.GRB.Status.OPTIMAL
        ...
        for linear_layer_count in range(num_linear_layers):
            ...
            for j in range(linear_layer.out_features):
                Wizi = gurobipy.LinExpr(
                    linear_layer.weight.data[j, :].tolist(), z[linear_layer_count]
                )
                ...
                if linear_layer_count < num_linear_layers - 1:
                    ...
                    A_z = A_pre.reshape((-1, 1)) @ linear_layer.weight.data[
                        j, :
                    ].reshape((1, -1))
                    z[linear_layer_count + 1][j] = prog.addVar(
                        lb=-gurobipy.GRB.INFINITY
                    )
                    A_relu = torch.hstack(
                        (A_z, A_z_next.reshape((-1, 1)), A_beta_i.reshape((-1, 1)))
                    )
                    var_relu = (
                    ...
                    z_next_j_objective = gurobipy.LinExpr(z[linear_layer_count + 1][j])
                    prog.setObjective(z_next_j_objective, sense=gurobipy.GRB.MAXIMIZE)
                    prog.optimize()
                    assert prog.status == gurobipy.GRB.Status.OPTIMAL
                    z_up[linear_layer_count + 1][j] = prog.ObjVal
                    prog.setObjective(z_next_j_objective, sense=gurobipy.GRB.MINIMIZE)
                    prog.optimize()
                    assert prog.status == gurobipy.GRB.Status.OPTIMAL
                    z_lo[linear_layer_count + 1][j] = prog.ObjVal
        return z_lo, z_up, Wz_lo, Wz_up, z

    def _compute_Wz_bounds_IA(
        self, vector_lower: torch.Tensor, vector_upper: torch.Tensor
    ):
        """
        This function is used in output_gradient_times_vector. It computes
        ...
        for layer_count in range(num_linear_layers):
            ...
            Wizi_lo = torch.zeros(linear_layer.weight.shape[0], dtype=self.dtype)
            Wizi_up = torch.zeros(linear_layer.weight.shape[0], dtype=self.dtype)

            for j in range(linear_layer.weight.shape[0]):
                for k in range(linear_layer.weight.shape[1]):
                    if linear_layer.weight[j][k] > 0:
                        Wizi_lo[j] += linear_layer.weight[j][k] * z_lo[layer_count][k]
                        Wizi_up[j] += linear_layer.weight[j][k] * z_up[layer_count][k]
                    else:
                        Wizi_lo[j] += linear_layer.weight[j][k] * z_up[layer_count][k]
                        Wizi_up[j] += linear_layer.weight[j][k] * z_lo[layer_count][k]
            ...
            if layer_count < num_linear_layers - 1:
                ...
                if isinstance(relu_layer, nn.ReLU):
                    ...
                    assert c >= 0
                    z_lo[layer_count + 1], _ = torch.min(
                        torch.cat(
                            (
                                Wz_lo[layer_count].reshape((1, -1)),
                                c * Wz_lo[layer_count].reshape((1, -1)),
                            ),
                            dim=0,
                        ),
                        axis=0,
                    )
                    z_up[layer_count + 1], _ = torch.max(
                        torch.cat(
                            (
                                Wz_up[layer_count].reshape((1, -1)),
                                c * Wz_up[layer_count].reshape((1, -1)),
                            ),
                            dim=0,
                        ),
                        axis=0,
