    ...
    if neuron_input_lo < 0 and neuron_input_up > 0:
        ...
        if isinstance(relu_layer, nn.ReLU):
            ...
            A_relu_input, A_relu_output, A_relu_beta, relu_rhs = (
                utils.replace_leaky_relu_mixed_integer_constraint(
                    relu_layer.negative_slope, neuron_input_lo, neuron_input_up, dtype
                )
            )
        ...
        if neuron_input_lo >= 0:
            ...
            slope = (
                ...
                if isinstance(relu_layer, nn.LeakyReLU)
                else 0.0
            )
            binary_value = 0.0
        Aeq_linear_input = torch.cat(
            (-slope * Wij.reshape((1, -1)), torch.zeros((1, Wij.numel()), dtype=dtype)),
            dim=0,
        )
        Aeq_neuron_output = torch.tensor([[1.0], [0]], dtype=dtype)
        Aeq_binary = torch.tensor([[0.0], [1.0]], dtype=dtype)
        rhs_eq = torch.stack((slope * bij, torch.tensor(binary_value, dtype=dtype)))

        Ain_linear_input = torch.cat(
            (Wij.reshape((1, -1)), -Wij.reshape((1, -1))), dim=0
        )
...
def _add_constraint_by_layer(
    ...
    bias = (
        ...
        else torch.zeros((linear_layer.out_features,), dtype=dtype)
    )
    for j in range(linear_layer.out_features):
        ...
        Ain_z_next.append(
            torch.zeros((rhs_in_j.numel(), linear_layer.out_features), dtype=dtype)
        )
        ...
        Aeq_z_next.append(
            torch.zeros((rhs_eq_j.numel(), linear_layer.out_features), dtype=dtype)
        )
    ...
    return (
        torch.cat(Ain_z_curr, dim=0),
        torch.cat(Ain_z_next, dim=0),
        torch.cat(Ain_binary, dim=0),
        torch.cat(rhs_in, dim=0),
        torch.cat(Aeq_z_curr, dim=0),
        torch.cat(Aeq_z_next, dim=0),
        torch.cat(Aeq_binary, dim=0),
        torch.cat(rhs_eq, dim=0),
        torch.stack(z_next_lo),
        torch.stack(z_next_up),
    )


def _add_constraint_to_program_by_layer(
    prog: gurobi_torch_mip.GurobiTorchMIP,
    linear_layer: monotonic_utils.LinearyLayer,
    relu_layer,
    ...
    relu_output_var = prog.addVars(linear_layer.out_features, lb=-gurobipy.GRB.INFINITY)
    relu_activation_var = prog.addVars(
        linear_layer.out_features, lb=0.0, ub=1.0, vtype=binary_var_type
    )
    for j in range(linear_layer.out_features):
        bij = (
            torch.tensor(0.0, dtype=linear_layer.weight.dtype)
            if linear_layer.bias is None
            else linear_layer.bias[j]
        )
        (
            Ain_linear_input,
            Ain_neuron_output,
            ...
            linear_layer.weight[j], bij, relu_layer, relu_input_lo[j], relu_input_up[j]
        )
        prog.addMConstr(
