...
class FeedbackSystem:
    ...
    def __init__(
        ...
        if isinstance(controller_network, torch.nn.Sequential):
            ...
            assert controller_network[-1].out_features == self.forward_system.u_dim
        elif isinstance(controller_network, torch.nn.Linear):
            assert controller_network.in_features == self.forward_system.x_dim
            assert controller_network.out_features == self.forward_system.u_dim
        else:
        ...
        if isinstance(self.controller_network, torch.nn.Sequential):
            self.controller_relu_free_pattern = relu_to_optimization.ReLUFreePattern(
    ...
    def _add_network_controller_mip_constraint_given_relu_bound(
        ...
        x_var,
        u_var,
        controller_pre_relu_lo,
        controller_pre_relu_up,
        network_input_lo,
        network_input_up,
        ...
        controller_slack, controller_binary = prog.add_mixed_integer_linear_constraints(
            controller_mip_cnstr,
            x_var,
            None,
            controller_slack_var_name,
        ...
        if isinstance(self.controller_network, torch.nn.Sequential):
            prog.addMConstr(
                ...
                b=relu_x_equilibrium - self.u_equilibrium - controller_mip_cnstr.Cout,
                name="controller_output",
            )

        u_pre_sat_lo = (
            controller_network_output_lo - relu_x_equilibrium + self.u_equilibrium
        )
        u_pre_sat_up = (
            controller_network_output_up - relu_x_equilibrium + self.u_equilibrium
        )
    ...
    def _add_network_controller_mip_constraint(
        self,
        mip,
        x_var,
        u_var,
    ...
    def _add_linear_controller_mip_constraint(self, mip, x_var, u_var, binary_var_type):
        ...
        u_pre_sat_lo = (
            controller_network_output_lo - network_at_x_equilibrium + self.u_equilibrium
        )
        u_pre_sat_up = (
            controller_network_output_up - network_at_x_equilibrium + self.u_equilibrium
        )
        u_lower_bound, u_upper_bound = _add_input_saturation_constraint(
            mip,
            u_var,
        ...
        return controller_slack, controller_binary, u_lower_bound, u_upper_bound
    ...
    def strengthen_controller_mip_constraint(
        ...
        x_var: list,
        controller_slack: list,
        controller_binary: list,
        controller_post_relu_lo,
        controller_post_relu_up,
    ):
        """
        ...
        linear_inputs = torch.tensor(
            [v.x for v in x_var] + [v.x for v in controller_slack], dtype=mip.dtype
        )
        relu_activations = torch.tensor(
            [v.x for v in controller_binary], dtype=mip.dtype
        )
        linear_inputs_lo = torch.cat(
            (torch.from_numpy(self.forward_system.x_lo_all), controller_post_relu_lo)
        )
        linear_inputs_up = torch.cat(
            (torch.from_numpy(self.forward_system.x_up_all), controller_post_relu_up)
        )
    ...
    def add_dynamics_mip_constraint(
        ...
        controller_mip_cnstr_return = self._add_controller_mip_constraint(
            mip,
            x_var,
            u,
            controller_slack_var_name,
        ...
        forward_dynamics_return = self.forward_system.add_dynamics_constraint(
            mip,
            x_var,
            x_next_var,
            u,
    ...
    def strengthen_dynamics_constraint(
        ...
        assert isinstance(controller_mip_cnstr_return, ControllerMipConstraintReturn)
        if isinstance(
            forward_dynamics_return, relu_system.ReLUDynamicsConstraintReturn
        ):
            ...
            (
                Ain_forward_nn_input,
                Ain_forward_slack,
                Ain_forward_binary,
                rhs_in_forward,
            ) = self.forward_system.dynamics_relu_free_pattern.strengthen_mip_at_point(
                (forward_nn_linear_inputs, forward_relu_activations),
                forward_nn_linear_inputs_lo,
                forward_nn_linear_inputs_up,
            )
        ...
        if controller_mip_cnstr_return.nn_input is not None:
            self.strengthen_controller_mip_constraint(
                mip,
                controller_mip_cnstr_return.nn_input,
    ...
    def compute_u_pre(self, x):
        return (
            self.controller_network(x)
            - self.controller_network(self.x_equilibrium)
            + self.u_equilibrium
        )
    ...
    def compute_u(self, x):
        ...
        u[n] = ϕᵤ(x[n]) - ϕᵤ(x*) + u*
        """
        u_pre_sat = self.compute_u_pre(x)
        if len(x.shape) == 1:
            u = torch.max(
            ...
            u = torch.max(
                torch.min(
                    u_pre_sat, torch.from_numpy(self.u_upper_limit).reshape((1, -1))
                ),
    ...
    def step_forward(self, x):
        u = self.compute_u(x)
        if isinstance(self.forward_system, hybrid_linear_system.HybridLinearSystem):
...
def _add_input_saturation_constraint(
    mip,
    u_var,
    u_pre_sat,
    u_lower_limit: np.ndarray,
    ...
    for i in range(len(u_var)):
        if np.isinf(u_lower_limit[i]) and np.isinf(u_upper_limit[i]):
            mip.addLConstr(
            ...
            assert not np.isinf(u_lower_limit[i]) and not np.isinf(u_upper_limit[i])
