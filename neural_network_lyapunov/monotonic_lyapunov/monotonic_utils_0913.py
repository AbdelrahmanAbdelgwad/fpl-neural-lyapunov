...
def get_monotic_params(p_num,epsilon,dtype=torch.float64,device='cpu'):
    ...
    G[0,0] = 0
    return torch.tensor(F_1.T,dtype=dtype).to(device),\
        torch.tensor(F_2,dtype=dtype).to(device),\
            torch.tensor(F_3,dtype=dtype).to(device),\
            torch.tensor(G.T,dtype=dtype).to(device)
            
def generate_partition_space(size_partition,dtype=torch.float64,size_in=2,seed=9,provided_v=None):
    # if size_in==1:
    ...
    axis_v_samples = np.vstack((np.eye(size_in),-np.eye(size_in)))
    if provided_v is None:
        # if size_in<=2:
        #     theta = np.linspace(0.,2.0*np.pi,size_partition+1-size_in*2)[:-1]
        #     v_samples = np.vstack((np.cos(theta), np.sin(theta))).T
        #     v_samples = np.concatenate((axis_v_samples,v_samples),axis=0)
        # else:
        np.random.seed(seed=seed)
        v_samples = np.random.rand(size_partition-size_in*2,size_in)-0.5
        v_samples = v_samples/np.linalg.norm(v_samples,axis=1)[:, np.newaxis]
        v_samples = np.concatenate((axis_v_samples,v_samples),axis=0)
    else:
        if provided_v.shape[0]*2+size_in*2<size_partition:
            np.random.seed(seed=seed)
            v_samples = np.random.rand(size_partition-provided_v.shape[0]*2-size_in*2,size_in)-0.5
            v_samples = v_samples/np.linalg.norm(v_samples,axis=1)[:, np.newaxis]
            v_samples = np.concatenate((axis_v_samples,provided_v,-provided_v,v_samples),axis=0)
        else: 
            idx = np.random.choice(provided_v.shape[0]*2, size_partition-size_in*2,replace=False)
...
class LinearyLayer(nn.Module):
    ...
    def __init__(self, case=1,partition_space=None,grad_limit=0.1,size_in=1, size_out=1, \
        size_partition=1,size_piecewise=1,device="cpu",dtype=None,x_eqlm=None):
...
def setup_monotonic_relu(size_out=1,
    ...
    v = generate_partition_space(size_partition,dtype=dtype,size_in=size_in,provided_v=provided_v)#.to(device)
    if symm_v_dir_flag:
    ...
    layers[0] = LinearyLayer(case=1,partition_space=partition_space,\
        grad_limit=epsilon,size_in=size_in, size_out=size_out,\
            size_partition=size_partition,size_piecewise=size_piecewise,\
                dtype=dtype,x_eqlm=x_eqlm,device=device)
...
def l_inf_generalized_as_mixed_integer_constraint(
        x_lo: torch.Tensor,
        x_up: torch.Tensor,
        M: torch.Tensor) -> gurobi_torch_mip.MixedIntegerConstraintsReturn:
    """
    ...
    ret.Ain_input = torch.cat(
        (M, -M, 
         -M, M), dim=0)
    nM = M.shape[0]
    ret.Ain_slack = torch.cat((-torch.ones((nM, 1), dtype=dtype), 
                               torch.ones((nM, 1), dtype=dtype),
                               -torch.ones((nM, 1), dtype=dtype), 
                               torch.ones((nM, 1), dtype=dtype)), dim=0)
    # max_x_up = torch.max(x_up)
    # min_x_lo = torch.min(x_lo)
    max_x_up_plus_minus = torch.max(torch.cat((x_up,-x_lo), dim=0))
    diag_plus = torch.diag(max_x_up_plus_minus - x_lo)
    diag_minus = torch.diag(max_x_up_plus_minus + x_up)
    ret.Ain_binary = torch.cat((torch.zeros((nM, nM*2), dtype=dtype), 
                                torch.cat((diag_plus,torch.zeros_like(diag_plus)), dim=1),
                                torch.zeros((nM, nM*2), dtype=dtype), 
                                torch.cat((torch.zeros_like(diag_minus),diag_minus), dim=1)), 
                               dim=0)
    ret.rhs_in = torch.cat((torch.zeros((nM, ), dtype=dtype),
                            max_x_up_plus_minus - x_lo,
