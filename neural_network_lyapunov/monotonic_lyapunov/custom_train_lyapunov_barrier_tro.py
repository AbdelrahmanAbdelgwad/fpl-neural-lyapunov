...
class Trainer:
    ...
    def solve_linf_box_mip(self):
        ...
        milp,final_l, f_l, x_sol = self.Linf_box_bisection_search(self.x_equilibrium,self.V_lambda,
        ...
        return linf_expand_max, linf_expand_max_milp,  x_max
    ...
    def total_loss(self, positivity_state_samples,
                   ...
                   linf_expand_mip_cost_weight) -> TotalLossReturn:
        """
        ...
        if linf_expand_mip_cost_weight != 0 and linf_expand_mip_cost_weight is not None:
            levelSet_linf_expand, levelSet_linf_expand_milp, x_max = \
    ...
    def get_levelSet_region(self, x_eqlm, lyapunov_levelSet=1.):
        ...
        if self.lyapunov_hybrid_system.add_l1_state:
            ...
            for i_direction in range(2):
                result_list.append(lyapunov_levelSet*R[i_direction,:])
                result_list.append(-lyapunov_levelSet*R[i_direction,:])
        ...
        while iter_count < self.max_iterations:
            ...
            if  total_loss_return.lyap_loss.derivative_mip_obj <\
                    self.lyapunov_derivative_convergence_tol and \
                        self.linf_expand_mip_cost_weight is None:
                if self.lyapunov_positivity_mip_cost_weight is None:
            ...
            if self.optimizer == "custom_optimizer":#"custom_optimizer":
                ...
                if  total_loss_return.lyap_loss.derivative_mip_obj <\
                    ...
                    if self.linf_expand_mip_cost_weight is not None:
                        grad_linf_norm_0 = np.linalg.norm(training_params[0].grad_linf.reshape(-1).detach().numpy())
                        grad_linf_norm_1 = np.linalg.norm(training_params[1].grad_linf.reshape(-1).detach().numpy())
                        if grad_linf_norm_0+grad_linf_norm_1<1e-20:
                            print("terminate since grad_linf_norm_0+grad_linf_norm_1 is: ", grad_linf_norm_0+grad_linf_norm_1)
                            self._save_network(iter_count,endFlag_save=True)
                            self._save_loss_history(iter_count,loss_values,\
                                time.time()-train_start_time,endFlag_save=True)
                            return (True, total_loss_return.loss.item(),
