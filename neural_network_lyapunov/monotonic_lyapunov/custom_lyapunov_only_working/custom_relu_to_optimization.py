...
import neural_network_lyapunov.monotonic_lyapunov.custom_relu_to_optimization_utils as\
...
class ReLUFreePattern:
    ...
    def _output_constraint_given_bounds(self, z_pre_relu_lo, z_pre_relu_up,
                                        x_lo, x_up, monotonic_relu_flag=True):
        ...
        if monotonic_relu_flag:
            symm_multi = int(self.model[0].symm_flag)+1
            Ain_input_aux = torch.zeros((self.num_relu_units, self.x_size),dtype=self.dtype)
            Ain_slack_aux = torch.zeros((self.num_relu_units, self.num_relu_units),dtype=self.dtype)
            Ain_binary_aux = torch.zeros((self.num_relu_units, self.num_relu_units),dtype=self.dtype)
            rhs_in_aux = torch.empty((self.num_relu_units, ),dtype=self.dtype)
            size_partition = self.model[0].size_partition * symm_multi
            size_piecewise = self.model[0].size_piecewise
            piece_inNum = size_piecewise-1
            
            # \beta_00+\beta_01+....+\beta_0p>=0 for each direction
            in_count = 0
            for i in range(size_partition):
                
                # \beta_00>=\beta_01>=....>=\beta_0p for each direction
                Ain_binary_aux[in_count:in_count+piece_inNum,(in_count+i):(in_count+i+piece_inNum)] =\
                    -torch.eye(piece_inNum, dtype=self.dtype)
                Ain_binary_aux[in_count:in_count+piece_inNum,(in_count+i+1):(in_count+i+piece_inNum+1)] +=\
                    torch.eye(piece_inNum, dtype=self.dtype)
                in_count += piece_inNum
            for i in range(size_partition):
                Ain_binary_aux[in_count+i,i*size_piecewise:(i+1)*size_piecewise] = -torch.ones(size_piecewise, dtype=self.dtype)
            rhs_in_aux[in_count::] = -torch.ones(size_partition, dtype=self.dtype)
            Ain_input = torch.concat((Ain_input,Ain_input_aux),dim=0)
            Ain_slack = torch.concat((Ain_slack,Ain_slack_aux),dim=0)
            Ain_binary = torch.concat((Ain_binary,Ain_binary_aux),dim=0)
            rhs_in = torch.concat((rhs_in,rhs_in_aux),dim=0)
