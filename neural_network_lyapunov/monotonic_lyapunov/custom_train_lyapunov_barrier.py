...
import neural_network_lyapunov.monotonic_lyapunov.l_options as l_options
...
from neural_network_lyapunov.monotonic_lyapunov.lbfgs import FullBatchLBFGS
    ...
    def step(self):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # grad_V = grad_V/(np.linalg.norm(grad_V)+1e-13)
                # grad_linf = grad_linf/(np.linalg.norm(grad_linf)+1e-13)

                # grad_product = grad_V @ grad_linf
                # if grad_product < 0.:
                #     u = grad_linf-(grad_linf@grad_V)*grad_V
                # else:
                #     u = grad_linf
                # u_opt = u/(np.linalg.norm(u)+1e-13)
                ...
                s = model.addMVar(1, lb=0.0, vtype=gurobipy.GRB.CONTINUOUS, name="s")
                if focusing_l:
                    # grad_V[grad_V<1e-13] = 0.
                    model.addConstr(grad_V @ u == s, name="grad_V")
                    # if np.linalg.norm(grad_V)>0.1:
                    #     model.addConstr(grad_V @ u >= 1e-2,
                    #                 name="grad_V")
                    # else:
                    #     model.addConstr(grad_V @ u >= 0.,
                    #                 name="grad_V")
                    model.addConstr(grad_linf @ u >= 0.0, name="grad_linf")
                    model.addConstr(u @ u <= 1.0, name="norm_u")
                    ...
                    #                 name="grad_V")
                    # else:
                    #     model.addConstr(grad_V @ u >= 1e-6,
                    #                 name="grad_V")
                    model.addConstr(u @ u <= 1.0, name="norm_u")
                    model.setObjective(grad_V @ u, gurobipy.GRB.MAXIMIZE)

                model.setParam(gurobipy.GRB.Param.OutputFlag, False)
                model.optimize()
                u_opt = u.X
                if len(p.data.shape) >= 2:
                    u_opt_torch = torch.tensor(
                        u_opt.reshape(p.data.shape[0], p.data.shape[1]),
                        dtype=p.data.dtype,
                    )
                else:
                    u_opt_torch = torch.tensor(
                ...
                p.data = p.data - group["lr"] * u_opt_torch
...
class Trainer:
    ...
    def __init__(self):
        ...
        self.lyapunov_positivity_sample_cost_weight = 1.0
        # The margin in hinge(-V(xⁱ))
        self.lyapunov_positivity_sample_margin = 0.0
        # The weight of hinge(dV(xⁱ) + ε V(xⁱ)) in the total loss
        self.lyapunov_derivative_sample_cost_weight = 1.0
        # The margin in hinge(dV(xⁱ) + ε V(xⁱ))
        self.lyapunov_derivative_sample_margin = 0.0
        # The weight of -min_x V(x) - ε₂ |x - x*|₁ in the total loss
        self.lyapunov_positivity_mip_cost_weight = 10.0
        ...
        self.lyapunov_derivative_mip_cost_weight = 10.0
        # The number of (sub)optimal solutions for the MIP
        # max_x dV(x) + εV(x)
        self.lyapunov_derivative_mip_pool_solutions = 1
        # The weight of max_x V(x) - min_y V(y) s.t x∈∂ℬ, y∈∂ℬ
        self.boundary_value_gap_mip_cost_weight = 0.0
        self.linf_expand_mip_cost_weight = 0.0
        self.linf_margin_mip_cost_weight = 0.0
        ...
        self.optimzer_scheduling = False
        ...
        self.lyapunov_derivative_mip_params = {gurobipy.GRB.Param.OutputFlag: False}

        self.barrier_derivative_mip_params = {gurobipy.GRB.Param.OutputFlag: False}
    ...
    def add_lyapunov(
        ...
        if self.barrier_system is not None:
            assert self.barrier_system.system is self.lyapunov_hybrid_system.system
    ...
    def add_barrier(self, barrier_system: barrier.Barrier, x_star, c, barrier_epsilon):
    ...
    def lyapunov_sample_loss(
        ...
        if (
            lyapunov_positivity_sample_cost_weight != 0
            and positivity_state_samples.shape[0] > 0
        ):
            positivity_sample_loss = (
                lyapunov_positivity_sample_cost_weight
                * self.lyapunov_hybrid_system.lyapunov_positivity_loss_at_samples(
                    self.x_equilibrium,
                    positivity_state_samples,
                    self.V_lambda,
            ...
            positivity_sample_loss = torch.tensor(0.0, dtype=dtype)
        if (
            lyapunov_derivative_sample_cost_weight != 0
            and derivative_state_samples.shape[0] > 0
        ):
            derivative_sample_loss = (
                lyapunov_derivative_sample_cost_weight
                * self.lyapunov_hybrid_system.lyapunov_derivative_loss_at_samples_and_next_states(
                    self.V_lambda,
            ...
            derivative_sample_loss = torch.tensor(0.0, dtype=dtype)

        return positivity_sample_loss, derivative_sample_loss

    def barrier_sample_loss(
        self,
        safe_state_samples: torch.Tensor,
        unsafe_state_samples: torch.Tensor,
        derivative_state_samples: torch.Tensor,
        safe_cost_weight: float,
        ...
        if safe_cost_weight != 0 and safe_state_samples.shape[0] > 0:
            safe_sample_loss = safe_cost_weight * utils.loss_reduction(
                torch.maximum(
                    -self.barrier_system.value(
                        safe_state_samples, self.barrier_x_star, self.barrier_c
                    ),
                    torch.tensor(0, dtype=dtype),
                ),
                self.sample_loss_reduction,
            )
        else:
        ...
        if unsafe_cost_weight != 0 and unsafe_state_samples.shape[0] > 0:
            unsafe_sample_loss = unsafe_cost_weight * utils.loss_reduction(
                torch.maximum(
                    self.barrier_system.value(
                        unsafe_state_samples, self.barrier_x_star, self.barrier_c
                    ),
                    torch.tensor(0, dtype=dtype),
                ),
                self.sample_loss_reduction,
        ...
        if derivative_cost_weight != 0 and derivative_state_samples.shape[0] > 0:
            derivative_state_samples_next = self.barrier_system.system.step_forward(
                derivative_state_samples
            )
            derivative_sample_loss = (
                derivative_cost_weight
                * self.barrier_system.derivative_loss_at_samples_and_next_states(
                    self.barrier_x_star,
                    self.barrier_c,
    ...
    def solve_positivity_mip(self):
        dtype = self.lyapunov_hybrid_system.system.dtype
        lyapunov_positivity_as_milp_return = (
            self.lyapunov_hybrid_system.lyapunov_positivity_as_milp(
                self.x_equilibrium,
                self.V_lambda,
                self.lyapunov_positivity_epsilon,
                R=self.R_options.R(),
        ...
        if self.lyapunov_positivity_mip_warmstart:
            self.lyapunov_positivity_last_x_adv = torch.tensor(
                [v.x for v in lyapunov_positivity_as_milp_return[1]], dtype=dtype
            )
        ...
        for solution_number in range(
            ...
            if not self.add_adversarial_state_only or (
                ...
                and lyapunov_positivity_mip.gurobi_model.PoolObjVal > 0
            ):
    ...
    def solve_lyap_derivative_mip(self):
        ...
        if self.derivative_mip_num_strengthen_pts == 0:
            lyapunov_derivative_as_milp_return = (
                self.lyapunov_hybrid_system.lyapunov_derivative_as_milp(
                    self.x_equilibrium,
                    self.V_lambda,
                    self.lyapunov_derivative_epsilon,
                    self.lyapunov_derivative_eps_type,
                    R=self.R_options.R(),
                    lyapunov_upper=self.lyapunov_upper,
            ...
            lyapunov_derivative_as_milp_return = (
                self.lyapunov_hybrid_system.strengthen_lyapunov_derivative_as_milp(
                    self.x_equilibrium,
                    self.V_lambda,
                    self.lyapunov_derivative_epsilon,
                    self.lyapunov_derivative_eps_type,
        ...
        if self.derivative_mip_strengthen_binary:
            self.lyapunov_hybrid_system.strengthen_lyapunov_derivative_milp_binary(
        ...
        if self.lyapunov_derivative_mip_warmstart:
            self.lyapunov_derivative_last_x_adv = torch.tensor(
                [v.x for v in lyapunov_derivative_as_milp_return.x], dtype=dtype
            )
            ...
            if not self.add_adversarial_state_only or (
                ...
                if isinstance(
                    ...
                    derivative_mip_adversarial_mode = np.argwhere(
                        np.array(
                            [v.xn for v in lyapunov_derivative_as_milp_return.gamma]
                        )
                        > 0.99
                    )[0][0]
                    derivative_mip_adversarial_next.append(
                        self.lyapunov_hybrid_system.system.step_forward(
                            torch.tensor(derivative_mip_adversarial[-1], dtype=dtype),
    ...
    def Linf_box_bisection_search(
        self, x_equilibrium, V_lambda, lyapunov_lower=0.0, lyapunov_upper=1.0, R=None
    ):
        def solve_optimization(
            x_equilibrium, V_lambda, roa_level, l, mip_pool_solutions=1
        ):
            milp_return = self.lyapunov_hybrid_system._construct_milp_for_roa_expand(
                x_equilibrium,
                V_lambda,
                lyapunov_lower=0.0,
            ...
            milp.gurobi_model.setParam(
                gurobipy.GRB.Param.PoolSolutions, mip_pool_solutions
            )
            ...
            x_sol = []
            for solution_number in range(
                np.min((mip_pool_solutions, milp.gurobi_model.solCount))
            ):
                milp.gurobi_model.setParam(
                    gurobipy.GRB.Param.SolutionNumber, solution_number
                )
                if milp.gurobi_model.PoolObjVal >= -1000.0:
                    x_sol.append([v.xn for v in milp_return[1]])
            return milp.gurobi_model.ObjVal, x_sol, milp

        a = 1e-3
        b = 30  # 30 #20
        L = torch.empty(1, dtype=torch.float64, requires_grad=True)
        f_a, _, _ = solve_optimization(
            x_equilibrium, V_lambda, lyapunov_upper, torch.ones_like(L) * a
        )
        f_b, _, _ = solve_optimization(
            x_equilibrium, V_lambda, lyapunov_upper, torch.ones_like(L) * b
        )
        # a<b, f(a)>0, f(b)<0
        tol = 1e-5
        max_iter = 400
        ...
        while i < max_iter:
            c = (a + b) / 2
            f_c, x_sol, milp = solve_optimization(
                x_equilibrium, V_lambda, lyapunov_upper, torch.ones_like(L) * c
            )
            if (b - a) / 2.0 < tol and np.absolute(f_c) < tol:
                final_l = c
                break
            if np.sign(f_c) == np.sign(f_a):
                ...
                f_b = f_c
            i += 1
        return milp, final_l, torch.ones_like(L) * final_l, f_c, x_sol

    def solve_linf_box_mip(self):
        """
        Solve the problem
        max_x v(x)-l||x-x*||_inf
        s.t v(x) <= r
        """
        dtype = self.lyapunov_hybrid_system.system.dtype

        milp, final_l, final_l_torch, f_l, x_sol = self.Linf_box_bisection_search(
            self.x_equilibrium,
            self.V_lambda,
            lyapunov_upper=self.lyapunov_upper,
            R=self.R_options.R(),
        ...
        linf_expand_max = milp.compute_linf_objective_from_mip_data_and_solution(
            solution_number=0, penalty=1e-13, l_star=final_l
        )
        linf_expand_max_milp = milp.gurobi_model.ObjVal
        dtype = self.lyapunov_hybrid_system.system.dtype
        x_max = x_sol  # torch.tensor([v.x for v in x], dtype=dtype)
        return linf_expand_max, linf_expand_max_milp, x_max, f_l, final_l, final_l_torch

    def solve_linf_margin_mip(self, L, x_star):
        """
        Solve the problem
        max_x V(x) − min_y V(y)
        s.t x∈∂ℬ, y∈∂ℬ
        ...
        dtype = self.lyapunov_hybrid_system.system.dtype
        milp_return = self.lyapunov_hybrid_system._construct_milp_linf_margin_mip(
            self.x_equilibrium,
            self.V_lambda,
            lyapunov_lower=0.0,
            lyapunov_upper=self.lyapunov_upper,
        ...
        dtype = self.lyapunov_hybrid_system.system.dtype
        x_max = torch.tensor([v.x for v in milp_return[1]], dtype=dtype)
        return V_max, V_max_milp, x_max
    ...
    def solve_boundary_gap_mip(self):
        ...
        V_coeff, V_vars, V_constant, _ = (
            self.lyapunov_hybrid_system._lyapunov_value_as_milp(
                milp, x, self.x_equilibrium, self.V_lambda, self.R_options.R()
            )
        )
        # The objective of this milp is V(x)
        milp.gurobi_model.setParam(gurobipy.GRB.Param.OutputFlag, False)
        # First maximize V(x)
        milp.setObjective(V_coeff, V_vars, V_constant, sense=gurobipy.GRB.MAXIMIZE)
        milp.gurobi_model.optimize()
        assert milp.gurobi_model.status == gurobipy.GRB.Status.OPTIMAL
        V_max = milp.compute_objective_from_mip_data_and_solution(
            solution_number=0, penalty=1e-13
        ...
        milp.setObjective(V_coeff, V_vars, V_constant, sense=gurobipy.GRB.MINIMIZE)
        milp.gurobi_model.optimize()
        assert milp.gurobi_model.status == gurobipy.GRB.Status.OPTIMAL
    ...
    def solve_barrier_value_mip(self, safe_flag):
        ...
        for region_count, region in enumerate(regions):
            ...
            mip_adversarial[region_count] = torch.empty(
                (num_adversarial, self.barrier_system.system.x_dim), dtype=dtype
            )
        ...
        for solution_number in range(num_adversarial):
            ...
            if not self.add_adversarial_state_only or (
                ...
                mip_adversarial.append(
                    torch.tensor([v.xn for v in barrier_deriv_return.x], dtype=dtype)
                )
        mip_adversarial = torch.stack(mip_adversarial)
        return barrier_deriv_return.milp, barrier_deriv_mip_obj, mip_adversarial
    ...
    class LyapLoss:
        def __init__(self):
            ...
            self.levelSet_linf_expand_loss = None
            self.levelSet_linf_margin_loss = None
    ...
    def compute_barrier_loss(
        ...
        if derivative_mip_cost_weight is not None:
            (
                derivative_mip,
                barrier_loss.derivative_mip_obj,
                derivative_mip_adversarial,
            ) = self.solve_barrier_derivative_mip()
            if derivative_mip_cost_weight != 0:
                barrier_loss.derivative_mip_loss = (
                    derivative_mip_cost_weight
                    * derivative_mip.compute_objective_from_mip_data_and_solution(
                        solution_number=0, penalty=1e-13
                    )
                )
        else:
        ...
        if (
            safe_mip_cost_weight is not None
            and safe_mip_adversarial is not None
            and len(safe_mip_adversarial) > 0
        ):
            barrier_loss.safe_state_samples = torch.cat(
                (safe_state_samples, torch.cat(safe_mip_adversarial, dim=0)), dim=0
            )
        if (
            unsafe_mip_cost_weight is not None
            and unsafe_mip_adversarial is not None
            and len(unsafe_mip_adversarial) > 0
        ):
            barrier_loss.unsafe_state_samples = torch.cat(
                (unsafe_state_samples, torch.cat(unsafe_mip_adversarial, dim=0)), dim=0
            )
        if (
            derivative_mip_cost_weight is not None
            and derivative_mip_adversarial is not None
        ):
        ...
        (
            ...
            barrier_loss.derivative_sample_loss,
        ) = self.barrier_sample_loss(
            barrier_loss.safe_state_samples[-self.max_sample_pool_size :],
            barrier_loss.unsafe_state_samples[-self.max_sample_pool_size :],
            barrier_loss.derivative_state_samples[-self.max_sample_pool_size :],
            safe_sample_cost_weight,
            unsafe_sample_cost_weight,
            derivative_sample_cost_weight,
        )
    ...
    def total_loss(
        ...
        linf_expand_mip_cost_weight,
        linf_margin_mip_cost_weight,
    ) -> TotalLossReturn:
        """
        ...
        if lyap_positivity_mip_cost_weight is not None:
            (
                lyap_positivity_mip,
                lyap_loss.positivity_mip_obj,
                positivity_mip_adversarial,
            ) = self.solve_positivity_mip()
        else:
        ...
        lyap_loss.positivity_mip_loss = torch.tensor(0.0, dtype=dtype)
        if (
            lyap_positivity_mip_cost_weight != 0
            and lyap_positivity_mip_cost_weight is not None
        ):
            lyap_loss.positivity_mip_loss = (
                lyap_positivity_mip_cost_weight
                * lyap_positivity_mip.compute_objective_from_mip_data_and_solution(
                    solution_number=0, penalty=1e-13
                )
            )
        ...
        lyap_loss.levelSet_linf_expand_loss = 0
        if linf_expand_mip_cost_weight != 0 and linf_expand_mip_cost_weight is not None:
            (
                levelSet_linf_expand,
                levelSet_linf_expand_milp,
                x_max,
                f_l,
                final_l,
                final_l_torch,
            ) = self.solve_linf_box_mip()
            print(f"levelSet_linf_expand: {levelSet_linf_expand}")
            print(f"derivative_mip_loss: {lyap_loss.derivative_mip_loss}")
            lyap_loss.levelSet_linf_expand_loss = (
                linf_expand_mip_cost_weight * levelSet_linf_expand
            )
        lyap_loss.levelSet_linf_margin_loss = 0
        if linf_margin_mip_cost_weight != 0 and linf_margin_mip_cost_weight is not None:
            if linf_expand_mip_cost_weight == 0 or linf_expand_mip_cost_weight is None:
                (
                    levelSet_linf_expand,
                    levelSet_linf_expand_milp,
                    x_max,
                ...
                print(
                    "levelSet_linf_expand: ",
                    levelSet_linf_expand,
                    " derivative_mip_lossL: ",
                    lyap_loss.derivative_mip_loss,
                )
            levelSet_linf_margin, levelSet_linf_expand_milp, x_max = (
                self.solve_linf_margin_mip(L=final_l_torch, x_star=x_max)
            )
            lyap_loss.levelSet_linf_margin_loss = (
                linf_margin_mip_cost_weight * levelSet_linf_margin
            )
        ...
        lyap_loss.derivative_state_samples_next = lyap_derivative_state_samples_next
        if (
            self.add_positivity_adversarial_state
            and lyap_positivity_mip_cost_weight is not None
        ):
        ...
        if (
            self.add_derivative_adversarial_state
            and lyap_derivative_mip_cost_weight is not None
        ):
            lyap_loss.derivative_state_samples = torch.cat(
                (lyap_derivative_state_samples, lyap_derivative_mip_adversarial), dim=0
            )
        ...
        if lyap_loss.positivity_state_samples.shape[0] > self.max_sample_pool_size:
            positivity_state_samples_in_pool = lyap_loss.positivity_state_samples[
                -self.max_sample_pool_size :
            ]
        else:
            positivity_state_samples_in_pool = lyap_loss.positivity_state_samples
        if lyap_loss.derivative_state_samples.shape[0] > self.max_sample_pool_size:
            lyap_derivative_state_samples_in_pool = lyap_loss.derivative_state_samples[
                -self.max_sample_pool_size :
            ]
            lyap_derivative_state_samples_next_in_pool = (
                lyap_loss.derivative_state_samples_next[-self.max_sample_pool_size :]
            )
        else:
            lyap_derivative_state_samples_in_pool = lyap_loss.derivative_state_samples
            lyap_derivative_state_samples_next_in_pool = (
        ...
        loss = (
            lyap_loss.positivity_sample_loss
    ...
    def _save_loss_history(
        self, iter_count, loss_history, time_used, endFlag_save=False
    ):
        if self.save_network_path:
            if iter_count % (self.save_network_iterations * 10) == 0 or endFlag_save:
                if endFlag_save:
                    print("save the final loss history")
                    print(
    ...
    def _training_params(self, search_L_only=False):
        """
        The parameters to be trained.
        """
        if not search_L_only:
            if (
                isinstance(
                    self.lyapunov_hybrid_system.system, feedback_system.FeedbackSystem
                )
                and self.search_controller
            ):
            ...
            training_params = self.L_options.variables()
        return training_params

    def get_levelSet_region(self, x_eqlm, lyapunov_levelSet=1.0):
        size_partition = self.lyapunov_hybrid_system.lyapunov_relu[0].size_partition
        size_piecewise = self.lyapunov_hybrid_system.lyapunov_relu[0].size_piecewise
        a = self.lyapunov_hybrid_system.lyapunov_relu[2].a.detach().numpy()
        b = self.lyapunov_hybrid_system.lyapunov_relu[0].b.detach().numpy()
        v = self.lyapunov_hybrid_system.lyapunov_relu[0].v.detach().numpy()
        # print(a.shape,b.shape,v.shape,lyapunov_relu_init[0].size_partition,x_des.shape)
        result_list = []
        for i_direction in range(size_partition):
            # in piecewise i, m(x) = \sum_{j<=i}a_j*(x-b_j)
            a_i = a[i_direction, :]
            ...
            # value of a monotic function at a set point
            # for first region (zero-th), set point value is zero
            # for ith region, set point value is \sum_{j<=i-1}a_j b_i  - \sum_{j<=i-1}a_j*(x-b_j)
            bk_points_val = np.multiply(a_i_sum[0:-1], b_i[1::])
            bk_points_val = np.concatenate(
                (np.array([0.0]), bk_points_val - a_b_prod_sum[0:-1])
            )

            # in which piecewise m(x) = r
            ncond = np.where(bk_points_val < lyapunov_levelSet)[0][-1]
            x = (lyapunov_levelSet + a_b_prod_sum[ncond]) / a_i_sum[ncond]
            result_list.append(x * v[i_direction, :])
            # print("direction: ", i_direction, ' condition: ', ncond)
        if self.lyapunov_hybrid_system.add_l1_state:
            R = self.R_options.R().detach().numpy()
            for i_direction in range(R.shape[0]):
                result_list.append(
                    lyapunov_levelSet
                    * R[i_direction, :]
                    / (self.V_lambda * np.linalg.norm(R[i_direction, :]))
                )
                # result_list.append(-lyapunov_levelSet*R[i_direction,:]/self.V_lambda)
        l_inf_rect = np.linalg.norm(np.array(result_list), np.inf, axis=0)
        bound_lo = []
        bound_up = []
        for i in range(self.x_equilibrium.shape[0]):
            bound_lo.append(x_eqlm[i] - l_inf_rect[i])
            bound_up.append(x_eqlm[i] + l_inf_rect[i])
        l_inf_bound_lo = torch.tensor(bound_lo, dtype=torch.float64)
        l_inf_bound_up = torch.tensor(bound_up, dtype=torch.float64)
        ...
        return l_inf_bound_lo, l_inf_bound_up
    ...
    def train(self, state_samples_all):
        ...
        assert state_samples_all.shape[1] == self.lyapunov_hybrid_system.system.x_dim
        ...
        if self.optimizer == "Adam":
            ...
            optimizer = external_optim.AdaBound(
                training_params,
            ...
            optimizer = external_optim.Adahessian(
                training_params,
                lr=self.learning_rate,
                betas=(0.9, 0.999),
                eps=1e-4,
        ...
        elif self.optimizer == "custom_optimizer":
            optimizer = MyOptimizer(training_params, lr=self.learning_rate)
        else:
            raise Exception("train: unknown optimizer, only support Adam or SGD.")
        if self.optimzer_scheduling:
            # scheduler = torch.optim.lr_scheduler.StepLR(optimizer,
            #        step_size = 4, # Period of learning rate decay
            #        gamma = 0.5)
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, "min", patience=5, factor=0.5, min_lr=1e-5
            )
        ...
        while iter_count < self.max_iterations:
            if self.lyapunov_upper is not None:
                l_inf_bound_lo, l_inf_bound_up = self.get_levelSet_region(
                    x_eqlm=self.lyapunov_hybrid_system.system.forward_system.x_equilibrium.detach().numpy(),
                    lyapunov_levelSet=self.lyapunov_upper,
                )
                self.lyapunov_hybrid_system.system.forward_system.x_lo = l_inf_bound_lo
                self.lyapunov_hybrid_system.system.forward_system.x_up = l_inf_bound_up
                self.lyapunov_hybrid_system.system.x_lo_all = (
                    self.lyapunov_hybrid_system.system.forward_system.x_lo_all
                )
                self.lyapunov_hybrid_system.system.x_up_all = (
                    self.lyapunov_hybrid_system.system.forward_system.x_up_all
                )
            self._save_network(iter_count)
            self._save_loss_history(
                iter_count, loss_values, time.time() - train_start_time
            )
            optimizer.zero_grad()
            if isinstance(
                self.lyapunov_hybrid_system.system, feedback_system.FeedbackSystem
            ...
            total_loss_return = self.total_loss(
                ...
                self.linf_expand_mip_cost_weight,
                self.linf_margin_mip_cost_weight,
            )
            positivity_state_samples = (
                ...
                wandb.log(
                    {
                        ...
                        "positivity MIP cost": total_loss_return.lyap_loss.positivity_mip_obj,
                        "derivative MIP cost": total_loss_return.lyap_loss.derivative_mip_obj,
                        "boundary gap MIP": total_loss_return.lyap_loss.gap_mip_loss,
                        "levelSet l_inf expand MIP": total_loss_return.lyap_loss.levelSet_linf_expand_loss,
                        "time": time.time() - train_start_time,
                    }
                )
            if self.output_flag:
                print(
                    f"Iter {iter_count}, loss {total_loss_return.loss}, "
                    + "positivity cost "
                    + f"{total_loss_return.lyap_loss.positivity_mip_obj}, "
                    + "derivative_cost "
                    + f"{total_loss_return.lyap_loss.derivative_mip_obj}, "
                    + "linf_expand_cost "
                    + f"{total_loss_return.lyap_loss.levelSet_linf_expand_loss}, "
                    + "linf_margin_cost "
                    + f"{total_loss_return.lyap_loss.levelSet_linf_margin_loss}"
                )
            ...
            if (
                ...
                and self.linf_expand_mip_cost_weight is None
                and self.linf_margin_mip_cost_weight is None
            ):
                if self.lyapunov_positivity_mip_cost_weight is None:
            ...
            # print(training_params[0].grad)
            # optimizer.step()
            # iter_count += 1
            print_mode = False
            if self.optimizer == "custom_optimizer":  # "custom_optimizer":
                option1 = (
                    ...
                    + total_loss_return.lyap_loss.levelSet_linf_margin_loss
                )
                option1.backward(retain_graph=True)
                for i in range(len(training_params)):
                    training_params[i].grad_V = training_params[i].grad.clone()
                    if print_mode:
                        print("grad_V:", training_params[i].grad)

                option2 = total_loss_return.lyap_loss.levelSet_linf_expand_loss
                option2.backward(retain_graph=True)
                for i in range(len(training_params)):
                    training_params[i].grad_linf = (
                        training_params[i].grad.clone() - training_params[i].grad_V
                    )
                    if print_mode:
                        print("grad_sum:", training_params[i].grad)
                        print("grad_V: ", training_params[i].grad_V)
                        print("grad_inf: ", training_params[i].grad_linf)
                grad_V_norm = 0.0
                for i in range(len(training_params)):
                    grad_V_norm += np.linalg.norm(
                        training_params[i].grad_V.reshape(-1).detach().numpy()
                    )
                # grad_V = grad_V.detach().numpy()
                # grad_linf = grad_linf.detach().numpy()
                # cos = grad_V@grad_linf/(np.linalg.norm(grad_V)*np.linalg.norm(grad_linf))
                print("total grad_V norm is: ", grad_V_norm)
                if (
                    total_loss_return.lyap_loss.derivative_mip_obj
                    < self.lyapunov_derivative_convergence_tol
                ):
                    if self.linf_expand_mip_cost_weight is not None:
                        if grad_V_norm < 1e-13:
                            print("terminate since total grad_V norm is: ", grad_V_norm)
                            self._save_network(iter_count, endFlag_save=True)
                            self._save_loss_history(
                                iter_count,
                                loss_values,
                ...
                total_loss_return.loss.backward(retain_graph=True)
            # print("gradient: ",training_params[0].grad,training_params[1].grad)
            if self.optimzer_scheduling:
                scheduler.step(total_loss_return.lyap_loss.derivative_mip_loss.clone())
                optimizer.step()
            else:
                optimizer.step()
            iter_count += 1
    ...
    def train_lineSearch(self, state_samples_all):
        ...
        optimizer = FullBatchLBFGS(training_params, line_search="Armijo")
        ...
        while iter_count < self.max_iterations:
            ...
            def closure():
                state_samples_all = torch.empty((0, 2), dtype=torch.float64)
                positivity_state_samples = state_samples_all.clone()
                derivative_state_samples = state_samples_all.clone()
                derivative_state_samples_next = torch.empty_like(state_samples_all)
                if self.lyapunov_upper is not None:
                    ...
                    self.lyapunov_hybrid_system.system.forward_system.x_lo = (
                        l_inf_bound_lo
                    )
                    self.lyapunov_hybrid_system.system.forward_system.x_up = (
                        l_inf_bound_up
                    )
                    self.lyapunov_hybrid_system.system.x_lo_all = (
                        self.lyapunov_hybrid_system.system.forward_system.x_lo_all
                ...
                optimizer.zero_grad()
                loss_linf = total_loss_return.lyap_loss.levelSet_linf_expand_loss
                # if backward:
                #     loss.backward(retain_graph=True)
                return loss_V, loss_linf, total_loss_return

            option1, option2, total_loss_return = closure()
            ...
            option2.backward(retain_graph=True)
            for i in range(len(training_params)):
                training_params[i].grad_linf = training_params[
                    i
                ].grad.clone()  # - training_params[i].grad_V
                if print_mode:
                    print("grad_sum:", training_params[i].grad)
                    print("grad_V: ", training_params[i].grad_V)
                    print("grad_inf: ", training_params[i].grad_linf)
            ...
            grad_V, grad_linf = optimizer._gather_flat_grad_custom()
            grad_V = grad_V.detach().numpy()
            grad_linf = grad_linf.detach().numpy()
            cos = (
                grad_V
                @ grad_linf
                / (np.linalg.norm(grad_V) * np.linalg.norm(grad_linf))
            )
            print(
                "total grad_V norm is: ",
                grad_V_norm,
                ", cosine(angle) between grad_V and grad_linf: ",
                cos,
            )
            if (
                total_loss_return.lyap_loss.derivative_mip_obj
        ...
        for epoch in range(num_epochs):
            ...
            for _, batch_data in enumerate(data_loader):
                ...
                total_loss_return = self.total_loss(
                    ...
                    boundary_value_gap_mip_cost_weight=None,
                    linf_expand_mip_cost_weight=None,
                )
    ...
    def _get_current_training_params(self):
        ...
        if not self.L_options.fixed_L:
            params["L_params"] = self.L_options._variables.clone()
        if isinstance(
            self.lyapunov_hybrid_system.system, feedback_system.FeedbackSystem
        ):
            params["controller_params"] = copy.deepcopy(
                self.lyapunov_hybrid_system.system.controller_network.state_dict()
            )
        return params
    ...
    def _set_training_params(self, params):
        ...
        if not self.L_options.fixed_L:
            self.L_options._variables = params["L_params"].clone()
        if isinstance(
            self.lyapunov_hybrid_system.system, feedback_system.FeedbackSystem
        ):
            self.lyapunov_hybrid_system.system.controller_network.load_state_dict(
    ...
    def _batch_descent_on_samples(
        ...
        best_loss = positivity_sample_initial_loss + derivative_sample_initial_loss
        best_training_params = self._get_current_training_params()
        if self.output_flag:
            print(
                "Before training, positivity_sample_loss "
        ...
        positivity_loader = torch.utils.data.DataLoader(
            positivity_dataset,
            batch_size=int(np.ceil(len(positivity_dataset) / options.num_batches)),
            shuffle=True,
        )
        derivative_loader = torch.utils.data.DataLoader(
            derivative_dataset,
            batch_size=int(np.ceil(len(derivative_dataset) / options.num_batches)),
            shuffle=True,
        )
        for epoch in range(options.num_epochs_per_mip):
            ...
            for i in range(np.min((len(positivity_loader), len(derivative_loader)))):
                optimizer.zero_grad()
                positivity_state_batch, positivity_state_repeatition_batch = next(
                    it_positivity_samples
                )
                derivative_state_batch, derivative_state_repeatition_batch = next(
                    it_derivative_samples
                )
            ...
            if self.output_flag:
                print(
                    f"epoch {epoch}, positivity_sample_loss "
                    + f"{positivity_sample_epoch_loss.item()}, "
                    + "derivative_sample_loss "
                    + f"{derivative_sample_epoch_loss.item()}"
                )
            if (
                positivity_sample_epoch_loss == 0.0
                and derivative_sample_epoch_loss == 0.0
            ):
                return
            if positivity_sample_epoch_loss + derivative_sample_epoch_loss < best_loss:
                best_training_params = self._get_current_training_params()
                best_loss = positivity_sample_epoch_loss + derivative_sample_epoch_loss
    ...
    def train_adversarial(
        self,
        positivity_state_samples_init: torch.Tensor,
        ...
        while iter_count < self.max_iterations:
            ...
            if not np.isinf(options.adversarial_cluster_radius):
                positivity_mip_adversarial, positivity_mip_adversarial_repeatition = (
                    _cluster_adversarial_states(
                        positivity_mip_adversarial, options.adversarial_cluster_radius
                    )
                )
                derivative_mip_adversarial, derivative_mip_adversarial_repeatition = (
                    _cluster_adversarial_states(
                        derivative_mip_adversarial, options.adversarial_cluster_radius
                    )
                )
            # else:
                ...
                derivative_mip_adversarial = torch.cat(
                    (
                        ...
                        (
                            ...
                            * options.perturb_derivative_sample_std
                        ).reshape((-1, derivative_mip_adversarial.shape[1])),
                    ),
                    dim=0,
                )
            positivity_state_samples_all = torch.cat(
                (positivity_state_samples_all, positivity_mip_adversarial), dim=0
            )
            derivative_state_samples_all = torch.cat(
                (derivative_state_samples_all, derivative_mip_adversarial), dim=0
            )
            ...
            if (
                ...
                return True, positivity_state_samples_all, derivative_state_samples_all
            ...
            self._batch_descent_on_samples(
                positivity_state_samples_all,
                derivative_state_samples_all,
                optimizer,
                positivity_state_repeatition,
...
class TrainValueApproximator:
    ...
    def train_with_cost_to_go(
        ...
        state_samples_all = torch.stack([pair[0] for pair in x0_value_samples], dim=0)
        ...
        for epoch in range(self.max_epochs):
            ...
            value_relu = (
                ...
                - relu_x_equilibrium
                + V_lambda
                * torch.norm(R @ (state_samples_all - x_equilibrium).T, dim=0, p=1)
            )
    ...
    def train(
        ...
        details.
        """
        assert isinstance(system, hybrid_linear_system.AutonomousHybridLinearSystem)
        assert isinstance(x_equilibrium, torch.Tensor)
        ...
        x0_value_samples = hybrid_linear_system.generate_cost_to_go_samples(
            system,
            [x0_samples[i] for i in range(x0_samples.shape[0])],
            T,
            instantaneous_cost_fun,
            discrete_time_flag,
            x_goal,
...
def _cluster_adversarial_states(adversarial_states, cluster_radius):
    ...
    if adversarial_states.shape[0] == 0:
        return adversarial_states, torch.tensor([], dtype=adversarial_states.dtype)
    with torch.no_grad():
        ...
        clustered_adversarial_states = torch.cat(
            (
                ...
                adversarial_states[1:][states_distance_squared > cluster_radius**2],
            ),
            dim=0,
        )
        new_adversarial_state_index = (
            torch.arange(adversarial_states.shape[0] - 1)[
                states_distance_squared > cluster_radius**2
            ]
            + 1
        )
        repeatition = new_adversarial_state_index[1:] - new_adversarial_state_index[:-1]
        if repeatition.shape == (0,):
            ...
            repeatition = torch.cat(
                (
                    torch.tensor(
                        [new_adversarial_state_index[0]], dtype=adversarial_states.dtype
                    ),
                    repeatition,
                    torch.tensor(
