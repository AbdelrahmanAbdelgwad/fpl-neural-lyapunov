...
class Trainer:
    ...
    def __init__(self):
        ...
        self.linf_expand_mip_cost_weight = 0.
        self.linf_margin_mip_cost_weight = 0.
    ...
    def add_lyapunov(
            self, lyapunov_hybrid_system: custom_lyapunov.LyapunovHybridLinearSystem,
    ...
    def Linf_box_bisection_search(self,x_equilibrium,V_lambda,lyapunov_lower=0., lyapunov_upper=1.,R=None):
        def solve_optimization(x_equilibrium,V_lambda,roa_level,l,mip_pool_solutions=1):
            milp_return = self.lyapunov_hybrid_system._construct_milp_for_roa_expand(x_equilibrium,
        ...
        L = torch.empty(1,
                        dtype=torch.float64,
                        requires_grad=True)
        f_a,_,_ = solve_optimization(x_equilibrium,V_lambda,lyapunov_upper,torch.ones_like(L)*a)
        f_b,_,_ = solve_optimization(x_equilibrium,V_lambda,lyapunov_upper,torch.ones_like(L)*b)
        # a<b, f(a)>0, f(b)<0
        tol = 1e-5
        max_iter = 400
        i = 0 
        ...
        while i < max_iter:
            c = (a+b)/2
            f_c, x_sol,milp= solve_optimization(x_equilibrium,V_lambda,lyapunov_upper,torch.ones_like(L)*c)
    ...
    def solve_linf_box_mip(self):
        ...
        milp,final_l, final_l_torch,f_l, x_sol = self.Linf_box_bisection_search(self.x_equilibrium,self.V_lambda,
                                                                  lyapunov_upper=self.lyapunov_upper,R=self.R_options.R())
    ...
    def solve_linf_margin_mip(self,L,x_star):
        ...
        dtype = self.lyapunov_hybrid_system.system.dtype
        milp_return = self.lyapunov_hybrid_system._construct_milp_linf_margin_mip(self.x_equilibrium,self.V_lambda,
                                                                                  lyapunov_lower=0., lyapunov_upper=self.lyapunov_upper,L=L, 
                                                                                  x_star=x_star,R=self.R_options.R())
    ...
    def total_loss(self, positivity_state_samples,
                   ...
                   linf_margin_mip_cost_weight) -> TotalLossReturn:
        """
        ...
        if lyap_derivative_mip_cost_weight != 0\
            ...
            lyap_loss.derivative_mip_loss = \
                lyap_derivative_mip_cost_weight * lyap_derivative_mip.\
                compute_objective_from_mip_data_and_solution(
                    solution_number=0, penalty=1e-13)
        ...
        if linf_expand_mip_cost_weight != 0 and linf_expand_mip_cost_weight is not None:
            levelSet_linf_expand, levelSet_linf_expand_milp, x_max, f_l,final_l,final_l_torch = \
        ...
        if linf_margin_mip_cost_weight != 0 and linf_margin_mip_cost_weight is not None:
            if linf_expand_mip_cost_weight == 0 or linf_expand_mip_cost_weight is None:
                levelSet_linf_expand, levelSet_linf_expand_milp, x_max, f_l,final_l,final_l_torch = \
                    self.solve_linf_box_mip()
                print("levelSet_linf_expand: ",levelSet_linf_expand, " derivative_mip_lossL: ", lyap_loss.derivative_mip_loss)
        ...
        loss = lyap_loss.positivity_sample_loss + \
            ...
            lyap_loss.gap_mip_loss + lyap_loss.levelSet_linf_expand_loss +\
            lyap_loss.levelSet_linf_margin_loss 

        return Trainer.TotalLossReturn(loss, lyap_loss, barrier_loss)

    def _save_loss_history(self, iter_count, loss_history, time_used, endFlag_save=False):
        if self.save_network_path:
            if iter_count % (self.save_network_iterations*10) == 0 or endFlag_save:
                if endFlag_save:
                    ...
                    print("total training time: ", round(time_used/60.,2),'minutes')
                dict = {"loss_history":loss_history,
                        "total_training_time":time_used}    
    ...
    def get_levelSet_region(self, x_eqlm, lyapunov_levelSet=1.):
        ...
        if self.lyapunov_hybrid_system.add_l1_state:
            ...
            for i_direction in range(R.shape[0]):
                result_list.append(lyapunov_levelSet*R[i_direction,:]/(self.V_lambda*np.linalg.norm(R[i_direction,:])))
    ...
    def train(self, state_samples_all):
        ...
        if self.optimizer == "Adam":
            ...
            optimizer = external_optim.AdaBound(training_params,
                                                lr= self.learning_rate,
                                                betas= (0.9, 0.999),
                                                final_lr = 0.1,
                                                gamma=1e-3,
            ...
            optimizer = external_optim.Adahessian(training_params,
                                                lr= self.learning_rate,
                                                betas= (0.9, 0.999),
                                                eps= 1e-4,
                                                weight_decay=0.0,
        ...
        while iter_count < self.max_iterations:
            if self.lyapunov_upper is not None:
                l_inf_bound_lo,l_inf_bound_up = self.get_levelSet_region(x_eqlm=self.lyapunov_hybrid_system.system.forward_system.x_equilibrium.detach().numpy(), 
                ...
                self.lyapunov_hybrid_system.system.x_lo_all =  self.lyapunov_hybrid_system.system.forward_system.x_lo_all
                self.lyapunov_hybrid_system.system.x_up_all = self.lyapunov_hybrid_system.system.forward_system.x_up_all
            self._save_network(iter_count)
            self._save_loss_history(iter_count,loss_values,time.time()-train_start_time)
            optimizer.zero_grad()
            if isinstance(self.lyapunov_hybrid_system.system,
                          feedback_system.FeedbackSystem):
            ...
            if self.enable_wandb:
                wandb.log({
                    ...
                    "levelSet l_inf expand MIP":
                    total_loss_return.lyap_loss.levelSet_linf_expand_loss,
            ...
            if  total_loss_return.lyap_loss.derivative_mip_obj <\
                    self.lyapunov_derivative_convergence_tol and \
                        self.linf_expand_mip_cost_weight is None and\
                        self.linf_margin_mip_cost_weight is None:
                if self.lyapunov_positivity_mip_cost_weight is None:
                    self._save_network(iter_count,endFlag_save=True)
                    self._save_loss_history(iter_count,loss_values,\
                        time.time()-train_start_time,endFlag_save=True)
                    return (True, total_loss_return.loss.item(),
                            total_loss_return.lyap_loss.positivity_mip_obj,
                            total_loss_return.lyap_loss.derivative_mip_obj)   
                elif total_loss_return.lyap_loss.positivity_mip_obj <=\
            ...
            if self.optimizer == "custom_optimizer":#"custom_optimizer":
                option1 = total_loss_return.lyap_loss.positivity_sample_loss + \
                                total_loss_return.lyap_loss.derivative_sample_loss + \
                                total_loss_return.lyap_loss.positivity_mip_loss + total_loss_return.lyap_loss.derivative_mip_loss +\
                                total_loss_return.lyap_loss.gap_mip_loss + total_loss_return.lyap_loss.levelSet_linf_margin_loss 
                ...
                for i in range(len(training_params)):
                    training_params[i].grad_linf  = training_params[i].grad.clone() - training_params[i].grad_V
                    if print_mode:
                ...
                grad_V_norm = 0.
                for i in range(len(training_params)):
                    grad_V_norm += np.linalg.norm(training_params[i].grad_V.reshape(-1).detach().numpy())
        ...
        while iter_count < self.max_iterations:
            ...
            def closure():
                ...
                if self.lyapunov_upper is not None:
                    ...
                    self.lyapunov_hybrid_system.system.x_lo_all =  self.lyapunov_hybrid_system.system.forward_system.x_lo_all
                    self.lyapunov_hybrid_system.system.x_up_all = self.lyapunov_hybrid_system.system.forward_system.x_up_all
                derivative_state_samples_next = torch.empty_like(derivative_state_samples)
                total_loss_return = self.total_loss(
                    positivity_state_samples, derivative_state_samples,
                    derivative_state_samples_next,
                    self.lyapunov_positivity_sample_cost_weight,
                ...
                derivative_state_samples_next = \
                    total_loss_return.lyap_loss.derivative_state_samples_next
                optimizer.zero_grad()
                loss_V = total_loss_return.lyap_loss.positivity_sample_loss + \
                        total_loss_return.lyap_loss.derivative_sample_loss + \
                        total_loss_return.lyap_loss.positivity_mip_loss + total_loss_return.lyap_loss.derivative_mip_loss +\
                        total_loss_return.lyap_loss.gap_mip_loss + total_loss_return.lyap_loss.levelSet_linf_margin_loss
                optimizer.zero_grad()
            ...
            cos = grad_V@grad_linf/(np.linalg.norm(grad_V)*np.linalg.norm(grad_linf))
            print("total grad_V norm is: ", grad_V_norm, 
                  ', cosine(angle) between grad_V and grad_linf: ',cos)
            if  total_loss_return.lyap_loss.derivative_mip_obj <\
                self.lyapunov_derivative_convergence_tol:
                if self.linf_expand_mip_cost_weight is not None:
                    if grad_V_norm<1e-13:
