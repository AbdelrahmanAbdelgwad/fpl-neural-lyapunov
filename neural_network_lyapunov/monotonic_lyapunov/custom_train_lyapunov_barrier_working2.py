    ...
    def step(self):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # p.data = p.data - 0.0001*u_opt_torch
            ...
            if self.optimizer == "custom_optimizer":#"custom_optimizer":
                if not debug_mode:
                    ...
                    if debug_mode_val == 1:
                        ...
                        option1.backward(retain_graph=True)
                        for i in range(len(training_params)):
                            training_params[i].grad_V  = training_params[i].grad
                        print(training_params[1].grad, training_params[1].grad_V)
                    else:
                        option2 = total_loss_return.lyap_loss.levelSet_linf_expand_loss
                        option2.backward(retain_graph=True)
                        for i in range(len(training_params)):
                            training_params[i].grad_linf  = training_params[i].grad 
                        # print(training_params[i].grad,training_params[i].grad_linf,training_params[i].grad_V)
                        print(training_params[1].grad, training_params[1].grad_linf)
                grad_V_norm = 0.
                for i in range(len(training_params)):
