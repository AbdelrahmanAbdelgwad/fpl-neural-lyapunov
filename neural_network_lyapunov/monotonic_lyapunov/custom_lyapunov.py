...
class LyapunovHybridLinearSystem:
    ...
    def lyapunov_value(self, x, x_equilibrium, V_lambda, *, R=None):
        ...
        if x.shape == (self.system.x_dim, ):
            # A single state.
            lyap_value = self.lyapunov_relu.forward(x) - relu_at_equilibrium
            lyap_value = lyap_value +\
                V_lambda * torch.norm(R @ (x - x_equilibrium), p=1) if self.add_l1_state else lyap_value
            return lyap_value
        else:
            # A batch of states.
            assert (x.shape[1] == self.system.x_dim)
            lyap_value = self.lyapunov_relu(x).squeeze() - \
                relu_at_equilibrium.squeeze()
            lyap_value = lyap_value+V_lambda * torch.norm(R @ (x - x_equilibrium).T, p=1,
                                            dim=0) if self.add_l1_state else lyap_value
            return lyap_value
    ...
    def _lyapunov_value_as_milp(self, mip, x, x_equilibrium, V_lambda,
        ...
        relu_at_equilibrium = self.lyapunov_relu.forward(x_equilibrium)
        V_coeff = [a_out.squeeze(0)]
        V_vars = [z]
        if self.add_l1_state:
            ...
            V_coeff.append(V_lambda * torch.ones(
    ...
    def lyapunov_positivity_as_milp(self,
        ...
        if self.add_l1_state:
            ...
            gamma) = self.add_state_error_l1_constraint(milp,
                                                        ...
                                                        slack_name="s",
                                                        binary_var_name="gamma")
            obj_coeff.append((V_epsilon - V_lambda) * torch.ones(
                (len(s), ), dtype=dtype))
            obj_vars.append(s)
        else:
            (s,
            gamma) = self.add_state_error_l1_constraint(milp,
                                                        ...
                                                        slack_name="s",
                                                        binary_var_name="gamma")
            obj_coeff.append((V_epsilon) * torch.ones(
                (len(s), ), dtype=dtype))
            obj_vars.append(s)
    ...
    def lyapunov_positivity_loss_at_samples(self,
        ...
        loss = self.lyapunov_value(
            ...
            R=R) 
        loss = loss - epsilon * torch.norm(
                R @ (state_samples - x_equilibrium).T, p=1, dim=0) if self.add_l1_state else loss
        if reduction == "mean":
    ...
    def add_lyapunov_bounds_constraint(self, lyapunov_lower, lyapunov_upper,
        ...
        if self.add_l1_state:
            ...
            const_coeff.append(V_lambda * torch.ones(
                    (s_dim, ), dtype=self.system.dtype))
            const_vars.append(state_error_s)
        if lyapunov_lower is not None:
            milp.addLConstr(const_coeff,const_vars,
    ...
    def compute_levelSet_boundary(self,V_lambda, R,x_equilibrium,x_lo_larger,x_up_larger):
        milp, x = self._construct_milp_for_roa_boundary(
            V_lambda, R, x_equilibrium,x_lo_larger.numpy(),x_up_larger.numpy())
    ...
    def _construct_milp_for_roa_boundary(self, V_lambda, R, x_equilibrium,x_lo_larger,x_up_larger):
        """
        ...
        # of the results.
        milp = gurobi_torch_mip.GurobiTorchMILP(dtype)
        x_lo = torch.from_numpy(x_lo_larger)
        x_up = torch.from_numpy(x_up_larger)
        x = milp.addVars(self.system.x_dim,
                         lb=x_lo,
                         ub=x_up,
                         vtype=gurobipy.GRB.CONTINUOUS,
        ...
        if self.add_l1_state:
            ...
            gamma) = self.add_state_error_l1_constraint(milp,
                                                        ...
                                                        slack_name="s",
                                                        binary_var_name="gamma")
            obj_coeff.append(V_lambda * torch.ones(
                (len(s), ), dtype=dtype))
            obj_vars.append(s)
...
class LyapunovDiscreteTimeHybridSystem(LyapunovHybridLinearSystem):
    ...
    def lyapunov_derivative_as_milp(self,
        ...
        if eps_type == ConvergenceEps.ExpLower or \
            ...
            obj_coeff = [a_out, (epsilon - 1) * a_out]
            obj_constant = epsilon * (b_out - relu_at_equilibrium.squeeze())
            obj_vars = [z_next, z]
            if self.add_l1_state:
                obj_coeff += [V_lambda * torch.ones((s_dim, ), dtype=self.system.dtype),\
                    (epsilon - 1) * V_lambda * torch.ones(
                        (s_dim, ), dtype=self.system.dtype)]
                obj_vars +=  [s_x_next_norm, s_x_norm]
            ...
            obj_coeff = [a_out, -a_out]
            obj_vars = [z_next, z]
            if self.add_l1_state:
                obj_coeff += [V_lambda * torch.ones(
                    (s_dim, ), dtype=self.system.dtype),\
                        (epsilon - V_lambda) * torch.ones(
                            (s_dim, ), dtype=self.system.dtype)]
                obj_vars += [s_x_next_norm, s_x_norm]
            milp.setObjective(obj_coeff, obj_vars, 0., gurobipy.GRB.MAXIMIZE)
        else:
    ...
    def add_roa_set_l_inf_constraint(self,
        ...
        if not torch.all(torch.from_numpy(self.system.x_lo_all)
            ...
            raise Exception("add_state_error_l1_constraint: we currently " +
                            "require that x_lo <= x_equilibrium <= x_up")
        mip_cnstr = monotonic_utils.l_inf_as_mixed_integer_constraint(
                        torch.from_numpy(self.system.x_lo_all),
                        torch.from_numpy(self.system.x_up_all))
    ...
    def _construct_milp_linf_margin_mip(self,x_equilibrium,V_lambda,lyapunov_lower=0., 
                                        lyapunov_upper=1.,L=None, x_star=None,R=None):
        """
        Solve the problem
        max_x V(x) − min_y V(y)
        s.t x∈∂ℬ, y∈∂ℬ
        ...
        f_l = self.lyapunov_relu.forward(x_star)+\
                V_lambda * torch.norm(R @ (
                    x_star.reshape(self.system.x_dim,) - x_equilibrium), p=1)
        milp = gurobi_torch_mip.GurobiTorchMILP(dtype)
        x = milp.addVars(self.system.x_dim,
                         lb=-gurobipy.GRB.INFINITY,
                         name="x")
        ...
        lx_lb, lx_ub = mip_utils.compute_range_by_IA(
            torch.eye(self.system.x_dim)*L, -x_equilibrium*L, 
            torch.from_numpy(self.system.x_lo_all),
            torch.from_numpy(self.system.x_up_all))
        ...
        mip_cnstr = monotonic_utils.l_inf_as_mixed_integer_constraint(
                        lx_lb,
                        lx_ub)
        mip_cnstr.transform_input(torch.eye(self.system.x_dim)*L, 
                                  -x_equilibrium*L)
        linf_slack, linf_alpha = milp.add_mixed_integer_linear_constraints(
            mip_cnstr, x, None, "linf_slack",
            "linf_alpha", "linf_ineq", "linf_eq", "",
            gurobipy.GRB.BINARY)
        
        milp.addLConstr([torch.ones((len(linf_slack),),dtype=dtype)], [linf_slack],
                sense=gurobipy.GRB.EQUAL,
                rhs= f_l)
        
        z, beta, a_out, b_out, _ = self.add_lyap_relu_output_constraint(milp, x)
        relu_at_equilibrium = self.lyapunov_relu.forward(x_equilibrium)
        ...
        if self.add_l1_state:
            # Now write the 1-norm |R*(x - x*)|₁ as mixed-integer linear
            # constraints.
            s, gamma = self.add_state_error_l1_constraint(milp,
                                                        x_equilibrium,
                                                        x,
                                                        R=R,
                                                        slack_name="s",
        ...
        self.add_lyapunov_bounds_constraint(lyapunov_lower,lyapunov_upper,
                    milp, a_out, b_out, V_lambda, z,
                    relu_at_equilibrium, s)
   
        obj_constant = b_out.squeeze() - relu_at_equilibrium.squeeze()
        
        # The objective of this milp is V(x)
        milp.gurobi_model.setParam(gurobipy.GRB.Param.OutputFlag, False)
        # First maximize V(x)
        milp.setObjective(obj_coeff,
                          obj_vars,
                          obj_constant,
                          sense=gurobipy.GRB.MAXIMIZE)
    ...
    def _construct_milp_for_roa_expand(self, x_equilibrium,V_lambda,lyapunov_lower=0., lyapunov_upper=1.,L=None, R=None):
        """
        ...
        s is the continuous slack variable -> s = lx, 
        alpha is the decision variable
        """
        # dtype = self.system.dtype
        # I could use the original gurobi interface directly, instead of the
        # gurobi_torch_mip interface, as we don't need to compute the gradient
        ...
        dtype = self.system.dtype
        milp = gurobi_torch_mip.GurobiTorchMILP(dtype)
        # l = torch.from_numpy(np.array([0.1]))
        x = milp.addVars(self.system.x_dim,
                        lb=-gurobipy.GRB.INFINITY,
                        vtype=gurobipy.GRB.CONTINUOUS,
                        name="x")
        ...
        if self.add_l1_state:
            ...
            s = None
        self.add_lyapunov_bounds_constraint(lyapunov_upper,lyapunov_upper,
                    milp, a_out, b_out, V_lambda, z,
                    relu_at_equilibrium, s)
        ...
        # L*||x-x*||_inf
        lx_lb, lx_ub = mip_utils.compute_range_by_IA(
            torch.eye(self.system.x_dim)*L, -x_equilibrium*L, torch.from_numpy(self.system.x_lo_all),
            torch.from_numpy(self.system.x_up_all))
        ...
        milp, exBox_s = self.excluding_box(milp,x,x_equilibrium)
        milp.addLConstr([torch.ones((len(exBox_s),),dtype=dtype)], [exBox_s],
                sense=gurobipy.GRB.GREATER_EQUAL,
                rhs= 1e-3)#0.05) #for pendulum example it is 0.2
        # milp.addLConstr(obj_coeff[0:-1], obj_vars[0:-1],
        #         sense=gurobipy.GRB.EQUAL,
        #         rhs= -obj_constant+lyapunov_upper)
                
        obj_vars.append(linf_slack)
        obj_coeff.append( -torch.ones(
                (len(linf_slack), ), dtype=dtype))
        milp.setObjective(obj_coeff, obj_vars,
    ...
    def excluding_box(self,milp,x,x_equilibrium):
        lx_lb, lx_ub = mip_utils.compute_range_by_IA(
            torch.eye(self.system.x_dim,dtype=self.system.dtype), -x_equilibrium, torch.from_numpy(self.system.x_lo_all),
            torch.from_numpy(self.system.x_up_all))
        
        mip_cnstr = monotonic_utils.l_inf_as_mixed_integer_constraint(
                        lx_lb,
                        lx_ub)
        mip_cnstr.transform_input(torch.eye(self.system.x_dim,dtype=self.system.dtype), 
                                  -x_equilibrium)
        s, exBox_alpha = milp.add_mixed_integer_linear_constraints(
            mip_cnstr, x, None, "exBox_slack",
            "exBox_alpha", "exBox_ineq", "exBox_eq", "",
            gurobipy.GRB.BINARY)
        # mip_cnstr = gurobi_torch_mip.MixedIntegerConstraintsReturn()
        
        # # The constraint is
        # # |x[i]-x_eqlm| > 0.1
        # s_dim = self.system.x_dim
        # R = torch.eye(s_dim,dtype=self.system.dtype)
        # Rx_lb, Rx_ub = mip_utils.compute_range_by_IA(
        #     R, -R @ x_equilibrium, torch.from_numpy(self.system.x_lo_all),
        #     torch.from_numpy(self.system.x_up_all))
        # s = [None] * s_dim
        ...
        #         (1, -1)), (-R[i, :] @ x_equilibrium).reshape((-1, )))
        #     s_i, alpha_i = milp.add_mixed_integer_linear_constraints(
        #         mip_cnstr, x, None, 'exBox_slack' + f"[{i}]",
        #         'exBox_binary'  + f"[{i}]", "exBox_ineq", "exBox_eq", "",
        #         gurobipy.GRB.BINARY)
        #     assert (len(s_i) == 1)
        #     s[i] = s_i[0]
        #     if len(alpha_i) == 1:
    ...
    def _construct_milp_for_generalized_roa_expand(self, x_equilibrium,V_lambda,lyapunov_lower=0., lyapunov_upper=1.,L=None, R=None):
        """
        Construct an MIP to solve the problem
        min l
        s.t V(x) - l||x||_\inf <= 0
        ...
        lx_lb, lx_ub = mip_utils.compute_range_by_IA(
            M*L, -M@x_equilibrium*L, torch.from_numpy(self.system.x_lo_all),
            torch.from_numpy(self.system.x_up_all))
        
        mip_cnstr = monotonic_utils.l_inf_as_mixed_integer_constraint(
                        lx_lb,
                        lx_ub)
        mip_cnstr.transform_input(M*L, 
                                  -M@x_equilibrium*L)
                # L*||x-x*||_inf
        M = torch.eye(self.system.x_dim,dtype=dtype)
        M = torch.vstack((M,torch.sum(M,dim=0)))
        lx_lb, lx_ub = mip_utils.compute_range_by_IA(
            M*L, -M@x_equilibrium*L, torch.from_numpy(self.system.x_lo_all),
            torch.from_numpy(self.system.x_up_all))
        
        mip_cnstr = monotonic_utils.l_inf_generalized_as_mixed_integer_constraint(
                        lx_lb,
                        lx_ub,
                        M)
        # mip_cnstr.transform_input(M*L, 
        #                           -M@x_equilibrium*L)
        mip_cnstr.transform_input(torch.eye(self.system.x_dim,dtype=dtype), 
                        -x_equilibrium)
    ...
    def _construct_milp_project_states(self, x_equilibrium,V_lambda, lyapunov_upper=1., R=None):
        ...
        # The objective of this milp is V(x)
        milp.gurobi_model.setParam(gurobipy.GRB.Param.OutputFlag, False)
        # First find ith dimension projected maximum
        milp.setObjective([1,0,0,0],
                           x ,
                          [0],
                          sense=gurobipy.GRB.MAXIMIZE)
        milp.gurobi_model.optimize()
        assert (milp.gurobi_model.status == gurobipy.GRB.Status.OPTIMAL)
        obj_max = milp.compute_objective_from_mip_data_and_solution(
            solution_number=0, penalty=1E-13)
        obj_max_milp = milp.gurobi_model.ObjVal
    ...
    #             # The lower and upper bound of R*(x-x*)
    #     # linf_slack_dim = M.shape[0]
    #     # Mx_lb, Mx_ub = mip_utils.compute_range_by_IA(
    #     #     M, -M @ x_equilibrium, torch.from_numpy(self.system.x_lo_all),
    #     #     torch.from_numpy(self.system.x_up_all))
    #     # linf_slack = [None] * linf_slack_dim
    #     # alpha = [None] * linf_slack_dim
    #     # for i in range(linf_slack_dim):
    #     #     mip_cnstr = monotonic_utils.l_inf_as_mixed_integer_constraint(
    #     #         Mx_lb[i], Mx_ub[i])
    #     #     # The constraint in mip_cnstr is
    #     #     # Ain_input * (R[i, :] * (x-x*)) + Ain_slack * s[i] +
    ...
    #     #     # Aeq_input * (R[i, :] * (x-x*)) + Aeq_slack * s[i] +
    #     #     # Aeq_binary * alpha = rhs_eq
    #     #     mip_cnstr.transform_input(M[i, :].reshape(
    #     #         (1, -1)), (-R[i, :] @ x_equilibrium).reshape((-1, )))
    #     #     s_i, alpha_i = milp.add_mixed_integer_linear_constraints(
    #     #         mip_cnstr, x, None, "linf_slack" + f"[{i}]",
    #     #         "linf_binary" + f"[{i}]", "linf_ineq", "linf_eq", "",
    #     #         False)
    #     #     assert (len(s_i) == 1)
    #     #     linf_slack[i] = s_i[0]
    #     #     if len(alpha_i) == 1:
    ...
    # def add_state_error_linf_constraint(self,
    #                                   milp,
    #                                   x_equilibrium,
    #                                   x,
    #                                   *,
    ...
    #         s_i, alpha_i = milp.add_mixed_integer_linear_constraints(
    #             mip_cnstr, x, None, slack_name + f"[{i}]",
    #             binary_var_name + f"[{i}]", "linf_ineq", "linf_eq", "",
    #             binary_var_type)
    #         assert (len(s_i) == 1)
    #         s[i] = s_i[0]
    #         if len(alpha_i) == 1:
    ...
    # def _construct_milp_rectFit(self, x_equilibrium,V_lambda,lyapunov_lower=0., lyapunov_upper=1.,L=None, R=None):
    #     """
    #     Construct an MIP to solve the problem
    #     min_{l,x} ||l||_1
    #     s.t V(x) - ||diag(l) x||_\inf <= 0
    #         V(x) <= r
    #         x \in V^{-1}(r)
    ...
    #                     vtype=gurobipy.GRB.CONTINUOUS,
    #                     name="x")
    #     l = milp.addVars(self.system.x_dim,
    #                     lb=1e-2,
    #                     vtype=gurobipy.GRB.CONTINUOUS,
    #                     name="l")

    #     # l bound
    #     l_lo = np.zeros_like(self.system.x_lo_all)
    #     l_up = np.ones_like(self.system.x_up_all)*20
    #     milp.addMConstr([np.eye(self.system.x_dim),-np.eye(self.system.x_dim)],
    #                                 [l,l],
    #                                 sense=gurobipy.GRB.LESS_EQUAL,
    #                                 b=[l_lo,
    #                                    l_up])
    ...
    #     # ||diag(L) * (x-x*)||_inf
    #     s_rect, gamma_rect = self.add_state_error_linf_constraint(milp,
    #                                                     x_equilibrium,
    #                                                     x,
    #                                                     R=l,
    #                                                     slack_name="s_rect",
    #                                                     binary_var_name="gamma_rect")
    ...
    #     for i in range(s_dim):
    #         mip_cnstr = utils.absolute_value_as_mixed_integer_constraint(
    #             l_lo[i], l_up[i], binary_for_zero_input=False)
    #         s_i, alpha_i = milp.add_mixed_integer_linear_constraints(
    #             mip_cnstr, x, None, 'rect_l1_' + f"[{i}]",
    #             'rect_binary' + f"[{i}]", "rect_l1_ineq", "rect_l1_eq", "",
    #             gurobipy.GRB.BINARY)
    #         assert (len(s_i) == 1)
    #         s[i] = s_i[0]
    #         if len(alpha_i) == 1:
    ...
    #     obj_coeff = [V_lambda * torch.ones((len(s), ), dtype=self.system.dtype)]   
    #     obj_vars = [s]
    #     milp.setObjective(obj_coeff, obj_vars,
    #                       constant=0.,
    #                       sense=gurobipy.GRB.MAXIMIZE)
