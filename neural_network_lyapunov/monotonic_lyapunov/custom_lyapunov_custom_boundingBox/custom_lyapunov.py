...
class LyapunovDiscreteTimeHybridSystem(LyapunovHybridLinearSystem):
    ...
    def _construct_milp_for_roa_expand(self, x_equilibrium,V_lambda,lyapunov_lower=0., lyapunov_upper=1.):
        """
        ...
        l = milp.addVars(1, vtype=gurobipy.GRB.CONTINUOUS, 
                         lb=1e-1,
                         name='l')
        ...
        mip_cnstr = monotonic_utils.l_inf_transformed_as_mixed_integer_constraint(
                        torch.from_numpy(self.system.x_lo_all),
                        torch.from_numpy(self.system.x_up_all),
                        x_equilibrium)
        linf_slack, linf_alpha = milp.add_mixed_integer_linear_constraints(
        ...
        # relu_at_zero = self.lyapunov_relu.forward(torch.zeros_like(x_equilibrium))
        milp.addQConstr([a_out.squeeze(), torch.tensor([[-1.]],dtype=dtype), torch.tensor([[1.]],dtype=dtype)],
                        [z, l, linf_slack],
                        sense=gurobipy.GRB.LESS_EQUAL,
                        rhs = -b_out + 1e-4*torch.ones_like(b_out)) # + 1e-4*torch.ones_like(b_out)
        relu_at_equilibrium = self.lyapunov_relu.forward(x_equilibrium)
        self.add_lyapunov_bounds_constraint(lyapunov_lower, lyapunov_upper,
                                    milp, a_out, b_out, V_lambda, z,
                                    relu_at_equilibrium, None)
                
        obj_coeff = [torch.tensor([[1.]],dtype=dtype)]
        obj_vars = [l]
