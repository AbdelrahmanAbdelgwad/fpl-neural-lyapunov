...
class LyapunovDiscreteTimeHybridSystem(LyapunovHybridLinearSystem):
    ...
    def _linf_gap_as_milp_original(self, mip, x, x_equilibrium, V_lambda,
                                R,L,lyapunov_lower=None,lyapunov_upper=None):
        """
        For an MILP, add the constraints such that we can compute
        V(x) = V_coeff * V_vars + V_constant
        ...
        mip_cnstr.transform_input(torch.eye(self.system.x_dim)*L, 
                                  -x_equilibrium*L)
        linf_slack, linf_alpha = mip.add_mixed_integer_linear_constraints(
            mip_cnstr, x, None, "linf_slack",
            "linf_alpha", "linf_ineq", "linf_eq", "",
            gurobipy.GRB.BINARY)
        obj_coeff.append( -torch.ones(
                (len(linf_slack), ), dtype=self.system.dtype))
        obj_vars.append(linf_slack)
        

        return obj_coeff, obj_vars, obj_constant, s, linf_slack


    def _linf_gap_as_milp(self, mip, x, x_equilibrium, V_lambda,
                                R,L,lyapunov_lower=None,lyapunov_upper=None):
        """
        For an MILP, add the constraints such that we can compute
        V(x) = V_coeff * V_vars + V_constant
        ...
        linf_eq_slack = mip.addVars(1,
                         lb=0.,
                         name="linf_eq_s")
        obj_coeff.append(torch.ones((1,),dtype=dtype))
        obj_vars.append(linf_eq_slack)
        mip.addLConstr(obj_coeff, obj_vars,
                       sense=gurobipy.GRB.EQUAL,
                        rhs= -b_out + relu_at_equilibrium)
        obj_coeff = [torch.ones((1,),dtype=dtype)]
        obj_vars = [linf_eq_slack]
        obj_constant = 0.#torch.zeros((1,),dtype=dtype)
        return obj_coeff, obj_vars, obj_constant, s, linf_slack
    ...
    def _construct_milp_for_roa_expand(self, x_equilibrium,V_lambda,lyapunov_lower=0., lyapunov_upper=1.,l=None):
        """
        ...
        obj_coeff = [
            a_out.squeeze(0), -torch.ones(
                (len(linf_slack), ), dtype=self.system.dtype)
        ]
        obj_vars = [z, linf_slack]
        obj_constant = b_out.squeeze() #- relu_at_equilibrium.squeeze()
        ...
        # linf_eq_slack = milp.addVars(1,
        #                  lb=0.,
        #                  name="linf_eq_s")
        # obj_coeff.append(torch.ones((1,),dtype=dtype))
        # obj_vars.append(linf_eq_slack)
        # milp.addLConstr(obj_coeff, obj_vars,
        #                sense=gurobipy.GRB.EQUAL,
        #                 rhs= -b_out + relu_at_equilibrium)
        # obj_coeff = [torch.ones((1,),dtype=dtype)]
        # obj_vars = [linf_eq_slack]
        ...
        milp.setObjective(obj_coeff, obj_vars,
                          ...
                          sense=gurobipy.GRB.MAXIMIZE)
        # obj_coeff = [-i for i in obj_coeff]
        # milp.setObjective(obj_coeff, obj_vars,
        #             constant=-obj_constant,
        #             sense=gurobipy.GRB.MINIMIZE)
