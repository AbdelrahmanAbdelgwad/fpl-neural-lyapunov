...
class Trainer:
    ...
    def __init__(self):
        ...
        self.linf_gap_mip_cost_weight = 0.
    ...
    def add_levelSet_linf_L(self,L_options):
        assert (isinstance(L_options, l_options.LOptions))
    ...
    def solve_linf_box_mip(self):
        ...
        obj_coeff, obj_vars, obj_constant, _,_= \
            self.lyapunov_hybrid_system._linf_gap_as_milp_original(
                milp, x, self.x_equilibrium, self.V_lambda,
                self.R_options.R(), self.L_options.L())
        ...
        linf_gap_max = milp.compute_objective_from_mip_data_and_solution(
            solution_number=0, penalty=1E-13)
        linf_gap_max_milp = milp.gurobi_model.ObjVal
        dtype = self.lyapunov_hybrid_system.system.dtype
        x_max = torch.tensor([v.x for v in x], dtype=dtype)
        return linf_gap_max, linf_gap_max_milp,  x_max
    ...
    class LyapLoss:
        def __init__(self):
            ...
            self.levelSet_linf_gap_loss = None
    ...
    class TotalLossReturn:
        def __init__(self, loss: torch.Tensor, lyap_loss, linf_gap_loss, barrier_loss):
            self.loss = loss
            assert (isinstance(lyap_loss, Trainer.LyapLoss))
            self.lyap_loss = lyap_loss
            self.linf_gap_loss = linf_gap_loss
    ...
    def total_loss(self, positivity_state_samples,
                   ...
                   linf_gap_mip_cost_weight) -> TotalLossReturn:
        """
        ...
        lyap_loss.levelSet_linf_gap_loss = 0     
        if linf_gap_mip_cost_weight != 0:
            levelSet_linf_gap, levelSet_linf_gap_milp, x_max = \
                self.solve_linf_box_mip()
            print(f"levelSet_linf_gap_gap: {levelSet_linf_gap}")
            lyap_loss.levelSet_linf_gap_loss = \
                linf_gap_mip_cost_weight * levelSet_linf_gap
        ...
        loss = lyap_loss.positivity_sample_loss + \
            ...
            lyap_loss.gap_mip_loss 
        linf_gap_loss = lyap_loss.levelSet_linf_gap_loss

        return Trainer.TotalLossReturn(loss, lyap_loss, linf_gap_loss, barrier_loss)
    ...
    def _save_network(self, iter_count, endFlag_save=False):
        if self.save_network_path:
            if iter_count % self.save_network_iterations == 0 or endFlag_save:
                ...
                torch.save(self.L_options.L(),
                           self.save_network_path + f'{"L.pt"}')
                if isinstance(self.lyapunov_hybrid_system.system,
        ...
        while iter_count < self.max_iterations:
            ...
            if self.enable_wandb:
                wandb.log({
                    ...
                    "levelSet l_inf gap MIP":
                    total_loss_return.lyap_loss.levelSet_linf_gap_loss,
    ...
    def train_linf_gap(self, state_samples_all):
        train_start_time = time.time()
        if self.output_flag:
            self.print()
        assert (isinstance(state_samples_all, torch.Tensor))
        ...
        if (state_samples_all.shape[0] > 0):
            ...
            derivative_state_samples_next = torch.empty_like(state_samples_all)
        iter_count = 0
        training_params = self._training_params(search_L_only=True)
        ...
        while iter_count < self.max_iterations:
            ...
            if self.output_flag:
                print(f"Iter {iter_count}, loss {total_loss_return.loss}, " +
                      ...
                      "derivative_cost " +
                      f"{total_loss_return.lyap_loss.derivative_mip_obj}")
            loss_values.append(total_loss_return.linf_gap_loss.detach())
            if  total_loss_return.linf_gap_loss <\
                    self.lyapunov_derivative_convergence_tol:
                self._save_network(iter_count,endFlag_save=True)
                self._save_loss_history(iter_count,loss_values,\
                    time.time()-train_start_time,endFlag_save=True)
            ...
            total_loss_return.linf_gap_loss.backward(retain_graph=True)
            optimizer.step()
            iter_count += 1
        return (False, total_loss_return.loss.item(),
                total_loss_return.lyap_loss.positivity_mip_obj,
