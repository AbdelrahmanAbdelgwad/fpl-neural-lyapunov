...
def generate_pendulum_dynamics_data(dt):
    ...
    print("theta_grid size: ",theta_grid.shape)
    ...
    return dataset_input,dataset_output#torch.utils.data.TensorDataset(dataset_input, dataset_output)
...
def train_forward_model(dynamics_model, model_dataset):
    ...
    def compute_next_v(model, state_action):
        ...
        return model(state_action) - model(
            torch.cat((state_equilibrium, control_equilibrium)).to(device))

    model_params = utils.train_approximator(v_dataset,
                             dynamics_model,
                             compute_next_v,
                             batch_size=256,
                             num_epochs=20,
...
def generate_controller_dataset():
    ...
    theta0 = np.linspace(-0.5 * np.pi, 0.5 * np.pi, 21)
    thetadot0 = np.linspace(-2, 2, 21)
    states = []
    controls = []
    costs = []
    dt = 0.01
...
def train_controller_approximator(state_samples, control_samples,
    ...
    def compute_control(model, dataset):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        return model(dataset) - model(state_equilibrium.to(device)) + control_equilibrium.to(device)

    model_params,model = utils.train_approximator(control_dataset,
                             controller_relu,
                             compute_control,
                             batch_size=64,
                             num_epochs=400,
...
def train_cost_approximator(state_samples, cost_samples, cost_relu, V_lambda, add_l1_state_flag):
    """
    ...
    def compute_cost(model, data):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        if add_l1_state_flag:
            return model(data) - model(state_equilibrium.to(device)) + V_lambda.to(device) * torch.norm(
                data - state_equilibrium.to(device), p=1, dim=1).reshape((-1, 1))
        else:
            return model(data) - model(state_equilibrium.to(device))

    model_params,model = monotonic_utils.train_approximator(cost_dataset,
                             cost_relu,
                             compute_cost,
                             batch_size=128,
                             num_epochs=300,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_dynamics_data", action="store_true")
    ...
    parser.add_argument("--bound_level",
                        type=int,default=11,help="bound level of x.")
    parser.add_argument("--bound_level_last",
                        type=int,default=10,help="bound level of x from pre-trained controller and lyapunov.")       
    ...
    if not search_controllerFalg:
        args.load_controller_relu = dir_path+"/data/monotonic_V06/monotonic_bound10_controller.pt"
    add_l1_state_flag = True#True
    lyap_symm_flag = False
    lyap_dir_symm_flag = False
    bound_level = args.bound_level
    bound_level_x = bound_level
    bound_level_y = bound_level
    
    if log_print_Flag:
        sys.stdout = open(dir_path+"/data/monotonic_roa_0929/"+"print_"+str(bound_level)+".txt", "a")
    ...
    if args.bound_level_last>=1:
        ...
        if search_controllerFalg:
            args.load_controller_relu=dir_path+"/data/monotonic_roa_0929/monotonic_bound"+str(bound_level_last)+"_controller.pt"
        if args.search_R:
            args.load_R=dir_path+"/data/monotonic_roa_0929/monotonic_bound"+str(bound_level_last)+"_R.pt"
            R = torch.load(args.load_R)
            R_options.set_variable_value(R.detach().numpy())
        args.load_lyapunov_relu=dir_path+"/data/monotonic_roa_0929/monotonic_bound"+str(bound_level_last)+"_lyapunov.pt"
    ...
    if args.generate_dynamics_data:
        data_input,data_output = generate_pendulum_dynamics_data(dt)
        tmp_dir_path= dir_path+ '/data/' + f'{"dynamics_dataset.pt"}'
        torch.save({"input":data_input,"output":data_output},tmp_dir_path)
        print("dynamic dataset has been saved to ",tmp_dir_path)
    elif args.load_dynamics_data:
        tmp_dir_path= dir_path+ '/data/' + f'{"dynamics_dataset.pt"}'
    ...
    if args.train_forward_model:
        print("train second order dynamics model.")
        model_params = train_forward_model(dynamics_model, model_dataset)
        tmp_dir_path = dir_path+ '/data/' + f'{"my_pendulum_second_order_forward.pt"}'
        torch.save(model_params,tmp_dir_path)
        print("save second order dynamics model to ",tmp_dir_path)
    else:
    ...
    if args.generate_controller_cost_data:
        ...
        tmp_dir_path = dir_path+ '/data/' + f'{"controller_cost_dataset.pt"}'
        torch.save({"state_samples":state_samples,
                    "control_samples":control_samples,
                    "cost_samples":cost_samples},tmp_dir_path)
        print("controller dataset has been saved to ",tmp_dir_path)
    elif args.load_controller_cost_data:
        controller_cost_data = torch.load(
            dir_path + "/data/controller_cost_dataset.pt")
    ...
    if args.train_controller_approximator:
        print("train controller model.")
        # tmp_dir_path = dir_path+ '/data/' + f'{"my_pendulum_controller.pt"}'
        # controller_relu = torch.load(tmp_dir_path)
        model_params,model =  train_controller_approximator(state_samples,
                                      control_samples,
                                      controller_relu,
                                      lr=0.001)
        tmp_dir_path = dir_path+ '/data/' + f'{"my_pendulum_controller.pt"}'
        torch.save(model.to('cuda'),tmp_dir_path)
        print("save controller model to ",tmp_dir_path)
    ...
    S_eig_value, S_eig_vec = np.linalg.eig(lqr_S)
    # R = torch.from_numpy(S_eig_value) + 0.01 * torch.eye(2, dtype=torch.float64)
    ...
    #                 symm_flag=lyap_symm_flag,
    #                 symm_v_dir_flag = lyap_dir_symm_flag)
    lyapunov_relu = monotonic_utils.setup_monotonic_relu(size_out=1,
                size_in=forward_system.x_equilibrium.numel(),
                epsilon=0.01,
                size_partition=6,
    ...
    if args.train_cost_approximator:
        print("train lyapunov model.")
        model_params,model = train_cost_approximator(state_samples, cost_samples, lyapunov_relu,
                                V_lambda,add_l1_state_flag)
        tmp_dir_path = dir_path+ '/data/' + f'{"my_pendulum_lyapunov.pt"}'
        torch.save(model.to('cuda'),tmp_dir_path)
        print("save lyapunov model to ",tmp_dir_path)
    ...
    lyapunov_hybrid_system.add_l1_state = add_l1_state_flag
    ...
    dut.lyapunov_derivative_convergence_tol = 3E-6
    ...
    dut.save_network_path = dir_path + "/data/monotonic_roa_0929/monotonic_bound" + str(bound_level) + "_"
    if args.train_adversarial:
        ...
        dut.optimzer_scheduling = False
        for i in range(3,8):
            dut.lyapunov_upper = 0.3*i#1.#None
            print("ROA value: ",dut.lyapunov_upper)
            dut.train(torch.empty((0, 2), dtype=torch.float64))
        dut.lyapunov_upper = 2.1#1.#None
        print("ROA value: ",dut.lyapunov_upper)
        dut.train(torch.empty((0, 2), dtype=torch.float64))
        
        num_of_trials = 25#5#15
        sub_num_of_trials = 3
        lyap_up_lo = 1
        dut.save_network_path = dir_path + "/data/monotonic_roa_0929/monotonic_bound" + str(bound_level+1) + "_"
        for m in range(2,2+num_of_trials):
            for j in range(sub_num_of_trials):
                dut.max_iterations = args.max_iterations
                dut.linf_expand_mip_cost_weight = None#0.001
                dut.linf_margin_mip_cost_weight = None #0.01
                dut.optimizer = "Adam"#"custom_optimizer"#"Adam""AdaBound"
                ...
                if m>2:
                    lyap_up_lo = 4
                for i in range(lyap_up_lo,8):
                    dut.lyapunov_upper = 0.3*i#1.#None
                    print("ROA value: ",dut.lyapunov_upper)
                    dut.train(torch.empty((0, 2), dtype=torch.float64))
                # compute_levelSet_boundary(self,V_lambda, R,x_equilibrium,x_lo_larger,x_up_larger)
                # if 
                    
                if j<sub_num_of_trials-1:
                    dut.max_iterations = 40#50
                    dut.lyapunov_upper = 2.1
                    ...
                    dut.train(torch.empty((0, 2), dtype=torch.float64))
            dut.save_network_path = dir_path + "/data/monotonic_roa_0929/monotonic_bound" + str(bound_level+m) + "_"
            print("save_network_path: ", dut.save_network_path)
