...
import neural_network_lyapunov.lyapunov as lyapunov
import neural_network_lyapunov.monotonic_lyapunov.monotonic_utils_pendulum as modified_utils
import gurobipy
import math
...
def plot_lyapunov_contour(x_lo,x_up,x_des,monotonic_NN=None,num_samples=[80,80],
                         V_lambda=0.8,add_l1_state=False,R=None,display_v=False,roa_level=None,
                          mode=None,plot_bLast=False,fig_name=None):
    x_lo_np = x_lo.detach().numpy()
    x_up_np = x_up.detach().numpy()
...
def plot_levelSet_Linf_region(x_lo,x_up,x_des,monotonic_NN=None,num_samples=[80,80],V_lambda=0.8,add_l1_state=False,
                         ...
                         x_star_list=None,report_flag=False,fig_name=None):
    x_lo_np = x_lo.detach().numpy()
    x_up_np = x_up.detach().numpy()
    (pos_mesh, vel_mesh) = np.meshgrid(np.linspace(x_lo_np[0], x_up_np[0], num_samples[0]),
                            np.linspace(x_lo_np[1], x_up_np[1], num_samples[1]))
