...
def train_forward_model(forward_model, model_dataset, num_epochs,save_dir=None):
    ...
    def compute_next_v(model, theta_thetadot_u):
        ...
        return model(theta_thetadot_u) - model(
            torch.cat(
                (torch.tensor([0, 0], dtype=torch.float64), u_equilibrium)).to(device))
...
def train_lqr_value_approximator(lyapunov_relu, V_lambda, R, x_equilibrium,
    ...
    def compute_v(model, x):
        ...
        return model(x) - model(x_equilibrium.to(device)) + (V_lambda * torch.norm(
            R.to(device) @ (x - x_equilibrium.reshape((1, 6)).to(device)).T, p=1, dim=0).reshape(
                (-1, 1)))
...
if __name__ == "__main__":
    ...
    if args.train_forward_model:
        forward_model = utils.setup_relu((4, 6, 6, 3),
                                         ...
                                         negative_slope=0.01,
                                         dtype=dtype)
        train_forward_model(forward_model, model_dataset, num_epochs=100,save_dir=args.load_forward_model)
