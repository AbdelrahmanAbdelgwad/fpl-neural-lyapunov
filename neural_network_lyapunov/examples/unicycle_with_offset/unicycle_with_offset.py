...
class Unicycle:
    ...
    def dynamics(self, x, u):
        ...
        if isinstance(x, np.ndarray):
            return np.array(
                [vel * np.cos(theta) - l * np.sin(theta)*thetadot, 
                 vel * np.sin(theta) + l * np.cos(theta)*thetadot, 
                 thetadot])
        elif isinstance(x, torch.Tensor):
            return torch.stack(
                (vel * torch.cos(theta) - l * torch.sin(theta)*thetadot,
                 vel * torch.sin(theta) + l * torch.cos(theta)*thetadot,
                 thetadot))
    ...
    def dynamics_gradient(self, x, u):
        ...
        if isinstance(x, np.ndarray):
            ...
            return np.array([[0, 0, -vel * sin_theta - l * cos_theta * thetadot], 
                             [0, 0, vel * cos_theta - l * sin_theta * thetadot],
                             [0, 0, 0]]), np.array([[cos_theta, -l * sin_theta],
                                                    [sin_theta, l * cos_theta], 
                                                    [0., 1]])
        elif isinstance(x, torch.Tensor):
            ...
            A[0, 2] = -vel * sin_theta - l * cos_theta*thetadot
            A[1, 2] = vel * cos_theta - l * sin_theta*thetadot
            B[0, 0] = cos_theta
            B[0, 1] = -l * sin_theta
            B[1, 0] = sin_theta
            B[1, 1] = l * cos_theta
...
class UnicycleReLUModel:
    ...
    where vel is the velocity along the unicycle heading direction.
    """
    ...
    def step_forward(self, x_start: torch.Tensor, u_start: torch.Tensor) ->\
        ...
        [pos_x[n+1], pos_y[n+1]] =
        [pos_x[n], pos_y[n]] + ϕ(θ[n], vel[n], θ_dot[n]) − ϕ(0, 0, 0)
        else
        [pos_x[n+1], pos_y[n+1]] =
        ...
        if len(x_start.shape) == 1:
            ...
            if self.thetadot_as_input:
                ...
                # network_input_zero = torch.cat((x_start[2:3],torch.zeros((2))),dim=0)
            else:
                network_input = torch.stack((x_start[2], u_start[0]))
                network_input_zero = torch.zeros((2, ), dtype=self.dtype)
                # network_input_zero = torch.cat((x_start[2:3],torch.zeros((1))),dim=0)
        ...
        mip.addMConstr([
            ...
            -mip_cnstr_result.Aout_slack
                        ...
                        b=mip_cnstr_result.Cout - self.dynamics_relu(
                            torch.zeros(
                                (network_input_size, ), dtype=self.dtype)),
...
class UnicycleReLUGivenThetaModel(UnicycleReLUModel):
    """
    We model the discrete-time dynamics of a unicycle using a multi-layer
    perceptron with (leaky) ReLU units.
    We consider the origin being the equilibrium state of the unicycle. The
    ...
    where vel is the velocity along the unicycle heading direction. Note that
    when vel=0, this model enforces that the position doesn't change.
    """
    def __init__(self, dtype, x_lo: torch.Tensor, x_up: torch.Tensor,
                 u_lo: torch.Tensor, u_up: torch.Tensor,
                 dynamics_relu: torch.nn.Sequential, dt: float,
        ...
        [θ, vel]
        """
        super(UnicycleReLUGivenThetaModel,
              self).__init__(dtype, x_lo, x_up, u_lo, u_up, dynamics_relu, dt,
                             thetadot_as_input)
    ...
    def step_forward(self, x_start: torch.Tensor, u_start: torch.Tensor) ->\
        ...
        when thetadot_as_input is true
        [pos_x[n+1], pos_y[n+1]] =
        [pos_x[n], pos_y[n]] + ϕ(θ[n], vel[n], θ_dot[n]) − ϕ(θ[n], 0, 0)
        else
        [pos_x[n+1], pos_y[n+1]] =
    ...
    def add_dynamics_constraint(self,
        ...
        if self.thetadot_as_input:
            ...
            network_input_zero_vel_lo = torch.stack(
                (self.x_lo[2], torch.tensor(0, dtype=self.dtype), torch.tensor(0, dtype=self.dtype)))
            network_input_zero_vel_up = torch.stack(
                (self.x_up[2], torch.tensor(0, dtype=self.dtype), torch.tensor(0, dtype=self.dtype)))
        else:
            network_input_lo = torch.stack((self.x_lo[2], u_lo[0]))
            network_input_up = torch.stack((self.x_up[2], u_up[0]))
            network_input_zero_vel_lo = torch.stack(
        ...
        if self.thetadot_as_input:
            input_vars = [x_var[2], u_var[0], u_var[1]]
            input_vars_zero_vel = [x_var[2], vel_zero_var, theta_zero_var]
        else:
            input_vars = [x_var[2], u_var[0]]
...
class UnicycleReLUZeroVelModel(UnicycleReLUModel):
    ...
    def __init__(self, dtype, x_lo: torch.Tensor, x_up: torch.Tensor,
        ...
        [θ, vel]
        """
        super(UnicycleReLUZeroVelModel,
              self).__init__(dtype, x_lo, x_up, u_lo, u_up, dynamics_relu, dt,
                             thetadot_as_input)
