...
class TestUnicycleFeedbackSystem(unittest.TestCase):
    def setUp(self):
        ...
        self.forward_system = unicycle.UnicycleReLUGivenThetaModel(
            self.dtype,
            self.x_lo,
            self.x_up,
            self.u_lo,
        ...
        self.dut = feedback_system.FeedbackSystem(
            self.forward_system, controller_network,
            self.forward_system.x_equilibrium,
            self.forward_system.u_equilibrium,
            self.u_lo.detach().numpy(),
            self.u_up.detach().numpy())
