...
def generate_dynamics_data(dt):
    ...
    x_samples =utils.uniform_sample_in_box(
            torch.tensor([x1_range[0], x2_range[0]],dtype=torch.float64),
            torch.tensor([x1_range[1], x2_range[1]],dtype=torch.float64), 1000).T
...
if __name__ == "__main__":
    ...
    if args.train_forward_model:
        ...
        dynamics_relu = utils.setup_relu((3, 6, 6, 2),
                                         ...
                                         bias=True,
                                         dtype=torch.float64)
        save_forward_dir = dir_path+'/../data/preprocess/path_following_unicycle_forward_model.pt'
