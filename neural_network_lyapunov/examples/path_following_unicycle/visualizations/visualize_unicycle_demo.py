...
x_lo = (
    torch.tensor([-0.8 * bound_level_tmp, -0.8 * bound_level_tmp], dtype=torch.float64)
    / 40.0
)
x_up = (
    torch.tensor([0.8 * bound_level_tmp, 0.8 * bound_level_tmp], dtype=torch.float64)
...
# load_prefix = "data/monotonic/monotonic_bound40_fpl/monotonic_bound40_fpl"
dir_path = "/home/abdelrahman/projects/Neural_Lyapunov_Control/neural-network-lyap-control-roa/neural_network_lyapunov/examples/path_following_unicycle/"  # os.path.dirname(os.path.realpath(__file__))#+"/.."
...
def setup_closed_loop_system(
    x_lo, x_up, dt, dynamics_model, controller_relu, lyapunov_relu, monotonic_flag=True
):
...
plot_loss_historySeq(dir_path + load_lossSeq_prefix, [*range(1, 40, 1)])
...
def plot_traj(
    ...
    for i in range(N_x):
        ...
        x_nextMat = (
            torch.cat(x_nextList, dim=0)
            .detach()
            .numpy()
            .reshape(len(x_nextList), x_dim)
        )
...
def plot_lqr_traj(
    ...
    for i in range(N_x):
        x = torch.tensor(xList[:, i], dtype=torch.float64) / 50.0
        x_nextList = [x]
        x_next_for_v = x_nextList[-1]
        # x_next_for_v = torch.cat(
        #         (lyapunov_hybrid_system.system.step_forward(x_nextList[-1])[0:2],
        ...
        while True:
            # print(lyapunov_hybrid_system.system.step_forward(x_nextList[-1])[0:2].shape)
            dtype = torch.float64
            u = torch.tensor(K, dtype=dtype) @ (
                x_nextList[-1] - torch.tensor(x_des, dtype=dtype)
            ) + torch.tensor(u_star, dtype=dtype)
            # print(K.shape,x_nextList[-1].shape,u.shape)
            x_next = forward_system.step_forward(
                x_nextList[-1].reshape(-1), u.reshape(-1)
            )  # lyapunov_hybrid_system.system.step_forward(x_nextList[-1])
            # print(x_next,closed_loop_system.compute_u(x_nextList[-1]))
            x_next_for_v = x_next
            # print(x_next)
            # x_next_for_v = torch.cat(
...
def get_bicycle_trajectories(x0):
    ...
    def bicycle_closed_loop_dynamics(
        plant: path_following.Unicycle,
        x: np.ndarray,
        controller_relu,
        x_equilibrium,
        u_equilibrium,
        ...
        #
        u_pre_saturation = (
            controller_relu(torch.from_numpy(x[[3, 4]]))
    ...
    def converged(t, y):
        dist = np.round(np.linalg.norm(y[[3, 4]] - x_des), 3) - 1e-2
        # print(dist)
        return dist
    ...
    result = scipy.integrate.solve_ivp(
        lambda t, x: bicycle_closed_loop_dynamics(
            plant,
            x,
            controller_relu,
...
def plot_lyapunov_contour(
    ...
    CS3 = ax2.contourf(X, Y, Z, origin=origin, extend="both")
    ...
    if roa_level is not None:
        ...
        CS5 = ax2.contour(X, Y, Z, roa_level, colors=("r",), linewidths=(2,))
        ax2.clabel(CS5, fmt="%.2f", colors="w", fontsize=14)
        contour_linewidth = 1
        contour_font = 10
    CS4 = ax2.contour(
        X, Y, Z, colors=("k",), linewidths=(contour_linewidth,), origin=origin
    )
    ...
    if display_v:
        ...
        scale = (
            min((x_up_np[0] - x_lo_np[0]) / 2.0, (x_up_np[1] - x_lo_np[1]) / 2.0) - 0.5
        )
        props = dict(boxstyle="round", facecolor="wheat", alpha=0.5)
        for v_i in range(v.shape[0]):
            ax2.text(
                x_des[0] + v[v_i, 0] * scale,
                x_des[1] + v[v_i, 1] * scale,
                str(v_i),
                fontsize=contour_font,
                bbox=props,
            )
            plt.arrow(
                x=x_des[0],
...
x_lo_larger = torch.tensor([-1, -1], dtype=torch.float64)
x_up_larger = torch.tensor([1, 1], dtype=torch.float64)
...
def find_roa(x_lo, x_up):
    ...
    x_lo_larger = x_lo
    x_up_larger = x_up
    rho = lyapunov_hybrid_system.compute_region_of_attraction(
        V_lambda, R, forward_system.x_equilibrium, None, x_lo_larger, x_up_larger
    )

    print(rho)
    milp1, _, _, _, _ = lyapunov_hybrid_system._construct_milp_for_roa(
        V_lambda, R, forward_system.x_equilibrium, x_lo_larger, x_up_larger, True
    )
...
plot_lyapunov_contour(
    ...
    mode="Trained",
    roa_level=[roa_level],
    fig_name="pf_tedrake_bound" + str(bound_level) + "_contour.png",
)
...
def compute_roa(all_points):
    ...
    def pf_closed_loop_dynamics(
        plant: path_following.Path_Following,
        x: np.ndarray,
        controller_relu,
        x_equilibrium,
        u_equilibrium,
    ...
    for idx in range(all_points.shape[0]):
        ...
        x0 = all_points[idx]
        result = scipy.integrate.solve_ivp(
            lambda t, x: pf_closed_loop_dynamics(
                plant,
                x,
                controller_relu,
                torch.from_numpy(x_des),
...
plt.tight_layout()
plt.savefig(
    "/home/zw2445/Documents/neural-network-lyapunov/plots/report/pf_tedrake_bound"
    + str(bound_level)
    + "_traj.png",
    dpi=300,
)
