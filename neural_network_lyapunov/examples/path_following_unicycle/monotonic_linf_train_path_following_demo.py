...
if __name__ == "__main__":
    ...
    x_lo = torch.tensor([-0.8*bound_level_x, -0.8*bound_level_y], dtype=torch.float64)/40.
    x_up = torch.tensor([0.8*bound_level_x, 0.8*bound_level_y], dtype=torch.float64)/40.
    ...
    V_lambda = 0.6
    controller_relu = utils.setup_relu((2, 4, 3, 1),
                                       params=None,
                                       negative_slope=0.1,
                                       bias=True,
                                       dtype=torch.float64)
    ...
    if args.train_adversarial:
        ...
        for i in range(1,8):
            dut.lyapunov_upper = 0.2*i#1.#None
        ...
        for m in range(2,2+num_of_trials):
            for j in range(sub_num_of_trials):
                ...
                if j<sub_num_of_trials-1:
                    dut.max_iterations = 50
                    dut.lyapunov_upper = 1.4
