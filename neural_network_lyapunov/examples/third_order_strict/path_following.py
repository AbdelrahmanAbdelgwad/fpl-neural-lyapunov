...
class Bicycle:
    ...
    def dynamics(self, x, u):
        ...
        if isinstance(x, np.ndarray):
            ...
            theta_der = self.v * np.tan(u[0])/self.l
            dist_e_der = self.v * np.sin(theta_e)
            theta_e_der = (self.v * np.tan(u[0])/self.l) - (self.v*np.cos(theta_e)/(1-dist_e_der))
            ...
            theta_der = self.v * torch.tan(u[0])/self.l
            dist_e_der = self.v * torch.sin(theta_e)
            theta_e_der = (self.v * torch.tan(u[0])/self.l) - (self.v*torch.cos(theta_e)/(1-dist_e_der))
...
class Third_Order_Strict:
    ...
    def dynamics(self, x, u):
        ...
        if isinstance(x, np.ndarray):
            x1_der = self.e1 * x2
            x2_der = self.e2 * x3
            x3_der = self.e3 * (x1**2) + self.e4 * u[0]
            return np.array([x1_der,x2_der,x3_der])
        elif isinstance(x, torch.Tensor):
            x1_der = self.e1 * x2
            x2_der = self.e2 * x3
            x3_der = self.e3 * (x1**2) + self.e4 * u[0]
            return torch.cat((x1_der.view(1), x2_der.view(1),x3_der.view(1)))
    ...
    def lqr_control(self, Q, R):
        ...
        A, B = self.dynamics_gradient(
            torch.tensor([0, 0, 0], dtype=self.dtype))
