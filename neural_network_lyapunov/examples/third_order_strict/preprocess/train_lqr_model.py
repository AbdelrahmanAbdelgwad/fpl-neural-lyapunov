...
def generate_dynamics_data(dt):
    ...
    x_samples = utils.uniform_sample_in_box(
        ...
        torch.tensor([x1_range[1], x2_range[1], x3_range[1]], dtype=torch.float64),
        1000,
    ).T
    u_samples = utils.uniform_sample_in_box(
        torch.full((1,), u_range[0], dtype=dtype),
        torch.full((1,), u_range[1], dtype=dtype),
        1000,
    ).T
    ...
    for x0 in x0s.T:
        ...
        x_next_tensors.append(torch.from_numpy(result.y[:, 1:]).T)
...
def train_forward_model(dynamics_model, model_dataset, num_epochs=20, save_dir=None):
    plant = path_following.Third_Order_Strict(dtype)
    state_equilibrium = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float64)
    control_equilibrium = torch.tensor([0.0], dtype=torch.float64)
    ...
    v_dataset = torch.utils.data.TensorDataset(xu_inputs, x_next_outputs)
...
def train_controller_approximator(
    ...
    def compute_u(model, x):
        ...
        return (
            ...
            + u_equilibrium.to(device)
            + lambda_u
            * torch.stack(
                (
                    torch.norm(Ru @ x[:, :2].T, p=1, dim=0),
                    torch.zeros((x.shape[0],), dtype=torch.float64),
                )
            ).T.to(device)
        )
...
def train_cost_approximator(lyapunov_relu, V_lambda, R, states, costs, num_epochs, lr):
...
if __name__ == "__main__":
    ...
    x_lo = torch.tensor(
        [-0.1 * bound_level_x, -0.1 * bound_level_y, -np.pi * bound_level_theta / 5],
        dtype=torch.float64,
    )
    x_up = torch.tensor(
        [0.1 * bound_level_x, 0.1**bound_level_y, np.pi * bound_level_theta / 5],
    ...
    controller_lambda_u = 4.0
    controller_Ru = torch.tensor(
        [[1, -1], [0, 1], [1, 0], [1, 1], [0.5, 0.9]], dtype=torch.float64
    )
    ...
    R = torch.cat(
        (
            ...
            torch.tensor([[1, -1, 0], [-1, -1, 1], [0, 1, 1]], dtype=torch.float64),
        ),
        dim=0,
    )
