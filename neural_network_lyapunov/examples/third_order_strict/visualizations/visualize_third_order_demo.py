...
plot_loss_historySeq(dir_path+load_lossSeq_prefix,[*range(1, 26, 1)])
...
def get_bicycle_trajectories(x0):
    x_des = np.zeros((2, ))
    def bicycle_closed_loop_dynamics(plant: path_following.Bicycle, x: np.ndarray,
    ...
    plant = path_following.Bicycle(torch.float64)
    q_equilibrium = torch.tensor([0.,0.], dtype=torch.float64)
    u_equilibrium = torch.tensor([np.arctan(plant.l)], dtype=torch.float64)
    # x_lo = torch.tensor([np.pi - 0.1 * np.pi, -0.5], dtype=torch.float64)
    # x_up = torch.tensor([np.pi + 0.1 * np.pi, 0.5], dtype=torch.float64)
    u_lo = torch.tensor([-np.pi*0.45], dtype=torch.float64)
    u_up = torch.tensor([np.pi*0.45], dtype=torch.float64)
    tspan = 600 
