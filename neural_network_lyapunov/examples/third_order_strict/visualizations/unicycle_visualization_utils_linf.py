...
def Linf_box_bisection_search(lyapunov_hybrid_system,forward_system,V_lambda,roa_level,bisection_search=True,R=None):
    ...
    L = torch.empty(1,
                    dtype=torch.float64,
                    requires_grad=True)
    f_a,_,_ = solve_optimization(lyapunov_hybrid_system,forward_system.x_equilibrium,V_lambda,roa_level,torch.ones_like(L)*a)
    f_b,_,_ = solve_optimization(lyapunov_hybrid_system,forward_system.x_equilibrium,V_lambda,roa_level,torch.ones_like(L)*b)
    # a<b, f(a)>0, f(b)<0
    tol = 1e-5
    max_iter = 400
    i = 0 
    ...
    while i < max_iter:
        c = (a+b)/2
        f_c, x_sol,milp= solve_optimization(lyapunov_hybrid_system,forward_system.x_equilibrium,V_lambda,roa_level,torch.ones_like(L)*c)
        if (b-a)/2. < tol and np.absolute(f_c)<tol:
            final_l = c
            break
        if np.sign(f_c) == np.sign(f_a):
