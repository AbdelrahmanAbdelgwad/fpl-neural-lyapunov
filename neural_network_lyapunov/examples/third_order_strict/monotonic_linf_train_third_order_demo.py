...
if __name__ == "__main__":
    ...
    if not search_controllerFalg:
        args.load_controller_relu = dir_path+"/data/monotonic/monotonic_bound50_controller.pt"
    ...
    V_lambda = 0.6
    controller_relu = utils.setup_relu((3, 4, 3, 1),
                                       params=None,
                                       negative_slope=0.1,
                                       bias=True,
                                       dtype=torch.float64)
    ...
    if args.train_adversarial:
        ...
        for m in range(2,2+num_of_trials):
            for j in range(sub_num_of_trials):
                ...
                dut.optimzer_scheduling = False
                for i in range(1,6):
                    dut.lyapunov_upper = 0.4*i#1.#None
                    print("ROA value: ",dut.lyapunov_upper)
                    dut.train(torch.empty((0, 3), dtype=torch.float64))
                    
                if j<sub_num_of_trials-1:
                    dut.max_iterations = 40
                    dut.lyapunov_upper = 2.0
