...
class Unicycle:
    ...
    def dynamics(self, x, u):
        ...
        theta = x[2]
        if isinstance(x, np.ndarray):
            x_dot = u[0] * np.cos(theta)
            y_dot = u[0] * np.sin(theta)
            theta_dot = u[1]
            return np.array([x_dot, y_dot, theta_dot])
        elif isinstance(x, torch.Tensor):
            x_dot = u[0] * torch.cos(theta)
            y_dot = u[0] * torch.sin(theta)
            theta_dot = u[1]
    ...
    def dynamics_gradient(self, x):
        ...
        A = torch.tensor(
            [[0, 0, -self.v0 * sin_theta], [0, 0, self.v0 * cos_theta], [0, 0, 0]],
            dtype=self.dtype,
        )

        B = torch.tensor([[cos_theta, 0], [sin_theta, 0], [0, 1]], dtype=self.dtype)
