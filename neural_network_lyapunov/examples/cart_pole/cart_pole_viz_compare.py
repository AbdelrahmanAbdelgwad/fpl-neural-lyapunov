...
def load_dynamics_model(dir_path, dt=0.01):
    ...
    bound_level = 10
    x_lo = torch.tensor(
        [-1 * bound_level, -np.pi/6 * bound_level, -1 * bound_level, -1 * bound_level],
        dtype=torch.float64
    ) / 100.0
    x_up = torch.tensor(
        [1 * bound_level, np.pi/6 * bound_level, 1 * bound_level, 1 * bound_level],
...
def load_models(bound_level, use_fpl=False):
    ...
    controller_relu = torch.load(controller_path, weights_only=False, map_location='cpu')
    
    # Load Lyapunov
    lyapunov_path = base_path + f"/monotonic_bound{bound_level}{suffix}_lyapunov.pt"
    lyapunov_relu = torch.load(lyapunov_path, weights_only=False, map_location='cpu')
    
    # Load R matrix
    R_path = base_path + f"/monotonic_bound{bound_level}{suffix}_R.pt"
    R = torch.load(R_path, weights_only=False, map_location='cpu')
    
    # Fix for monotonic layer: ensure all module attributes have gradients enabled
    # The monotonic layer has a non-standard implementation that calls reset_parameters()
    # in forward(), which then tries to retain_grad() on tensors
    for module in lyapunov_relu.modules():
        # Enable gradients for all tensor attributes
        for name, attr in module.__dict__.items():
            if isinstance(attr, torch.Tensor):
                attr.requires_grad = True
    
    # Also enable for standard parameters
    for param in lyapunov_relu.parameters():
        param.requires_grad = True
    for param in controller_relu.parameters():
        param.requires_grad = True
...
def compute_convergence_regions_vectorized(configurations, n_samples, x_lo, x_up,
    ...
    # Sample initial conditions - for cart-pole we'll focus on position and angle
    np.random.seed(42)  # For reproducibility
    x_samples = np.random.uniform(x_lo[0].item(), x_up[0].item(), n_samples)
    theta_samples = np.random.uniform(x_lo[1].item(), x_up[1].item(), n_samples)
    # Set velocities to zero for visualization
    x_dot_samples = np.zeros(n_samples)
    theta_dot_samples = np.zeros(n_samples)
    ...
    for controller, dynamics_type, title in configurations:
        ...
        for batch_idx in range(n_batches):
            ...
            for i in range(start_idx, end_idx):
                x0 = np.array([x_samples[i], theta_samples[i], 
                              x_dot_samples[i], theta_dot_samples[i]])
                
                try:
                    if dynamics_type == 'Analytical':
                        conv, _ = simulate_trajectory_analytical_fast(
                            plant, controller, x_equilibrium, u_equilibrium,
                            u_lo, u_up, x0, T=100.0, check_convergence=True
                        )
                        converged[i] = conv
                    else:
        ...
        results[title] = (x_samples, theta_samples, converged)
...
def create_full_comparison_plots_optimized(bound_level=3, V_lambda=0.6):
    ...
    u_lo = torch.tensor([-30], dtype=dtype)
    u_up = torch.tensor([30], dtype=dtype)
    ...
    # Test initial conditions within the verification region
    x_lo = torch.tensor(
        [-1 * bound_level, -np.pi/6 * bound_level, -1 * bound_level, -1 * bound_level],
        dtype=torch.float64
    ) / 100.0
    ...
    test_x0s = [
        np.array([x_up[0].item()/3, x_up[1].item()/3, 0., 0.]),  # Top-right corner
        np.array([x_lo[0].item()/3, x_lo[1].item()/3, 0., 0.])   # Bottom-left corner
    ]

    # 1. Phase portraits (x vs theta)
    ...
    test_x = np.linspace(x_lo[0].item(), x_up[0].item(), 2)
    test_theta = np.linspace(x_lo[1].item(), x_up[1].item(), 2)
    ...
    for controller, name in [(controller_std, 'Standard'), (controller_fpl, 'FPL')]:
        ...
        for x_val in test_x:
            for theta_val in test_theta:
                x0 = np.array([x_val, theta_val, 0., 0.])
                t, y, u = simulate_trajectory_analytical_fast(
                    plant, controller, x_equilibrium, u_equilibrium,
                    u_lo, u_up, x0, T=25.0
                )
                
                errors = np.sqrt(y[0]**2 + y[1]**2 + y[2]**2 + y[3]**2)
    ...
    # 5. Convergence regions with scatter plot (x-theta plane)
    print("Computing convergence regions...")
    axes_conv = [fig.add_subplot(gs[2, i]) for i in range(4)]
    
    # Define test region
    x_lo_test = torch.tensor(
        [-1.5 * bound_level, -np.pi/6 * 1.5 * bound_level, -1.5 * bound_level, -1.5 * bound_level],
        dtype=torch.float64
    ) / 100.0
    ...
    for ax_idx, (_, _, title) in enumerate(configurations):
        x_vals, theta_vals, converged = results[title]
        ...
        # Plot points
        ax = axes_conv[ax_idx]
        ax.scatter(x_vals[mask_not_conv], theta_vals[mask_not_conv], 
                  c='red', s=10, alpha=0.7, edgecolors='none', label='Does not converge')
        ax.scatter(x_vals[mask_conv], theta_vals[mask_conv], 
        ...
        rect_width = (x_up[0] - x_lo[0]).item()
        rect_height = (x_up[1] - x_lo[1]).item()
        rect = Rectangle((x_lo[0].item(), x_lo[1].item()), 
    ...
    fig.suptitle(f'Cart-Pole Controller Comparison (Bound Level {bound_level})', 
...
if __name__ == "__main__":
    ...
    if args.save_figs:
        fig.savefig(f'cart_pole_comparison_optimized_bound{args.bound_level}.png', dpi=300, bbox_inches='tight')
        print(f"\nSaved to cart_pole_comparison_optimized_bound{args.bound_level}.png")
