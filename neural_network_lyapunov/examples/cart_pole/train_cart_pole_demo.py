...
def path_following_closed_loop_dynamics(
    plant: cart_pole.Cart_Pole,
    x: np.ndarray,
    controller_relu,
    x_equilibrium,
    u_equilibrium,
...
if __name__ == "__main__":
    ...
    if args.bound_level_last >= 1:
        ...
        args.load_lyapunov_R = (
            dir_path + "/data/tedrake/tedrake_bound" + str(bound_level_last) + "_R.pt"
        )
    ...
    dynamics_relu = utils.setup_relu(
        (4, 7, 6, 2), params=None, negative_slope=0.01, bias=True, dtype=torch.float64
    )
    ...
    controller_relu = utils.setup_relu(
        (4, 5, 4, 1), params=None, negative_slope=0.1, bias=True, dtype=torch.float64
    )
    ...
    lyapunov_relu = utils.setup_relu(
        (4, 8, 7, 5, 1), params=None, negative_slope=0.1, bias=True, dtype=torch.float64
    )
    dtype = torch.float64
    x_star = np.zeros((4,))
    u_star = np.zeros((1,))
    ...
    u_lo = torch.tensor([-30.0], dtype=torch.float64)
    u_up = torch.tensor([30.0], dtype=torch.float64)
    ...
    dut.save_network_path = (
        dir_path + "/data/tedrake/tedrake_bound" + str(bound_level) + "_"
    )
