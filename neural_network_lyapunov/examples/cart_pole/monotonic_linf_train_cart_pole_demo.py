...
if __name__ == "__main__":
    ...
    parser.add_argument("--bound_level",
                        type=int,default=4,help="bound level of x.")
    ...
    if args.bound_level_last>=1:
        ...
        args.load_controller_relu=dir_path+"/data/monotonic_roa_0913/monotonic_bound"+str(bound_level_last)+"_controller.pt"
        args.load_lyapunov_relu=dir_path+"/data/monotonic_roa_0913/monotonic_bound"+str(bound_level_last)+"_lyapunov.pt"
        args.load_lyapunov_R=dir_path+"/data/monotonic_roa_0913/monotonic_bound"+str(bound_level_last)+"_R.pt"
        print("pre-trained bound level is: ", bound_level_last)
    else:
        # args.load_controller_relu=dir_path+"/data/preprocess/lqr_controller.pt"
        # args.load_lyapunov_relu=dir_path+"/data/preprocess/lqr_lyapunov_monotonic.pt"
        # args.load_lyapunov_R=dir_path+"/data/preprocess/lqr_R_monotonic.pt"
        # args.load_lyapunov_R= None
        tmp_bound = 100
        args.load_controller_relu=dir_path+"/data/monotonic/monotonic_bound"+str(tmp_bound)+"_controller.pt"
        args.load_lyapunov_relu=dir_path+"/data/monotonic/monotonic_bound"+str(tmp_bound)+"_lyapunov.pt"
        args.load_lyapunov_R=dir_path+"/data/monotonic/monotonic_bound"+str(tmp_bound)+"_R.pt"
        print("load from LQR ")
    print("pretrained lyapunov path: ",args.load_lyapunov_relu)
    x_lo = torch.tensor([-1*bound_level,-np.pi/6*bound_level,
                         -1*bound_level,-1*bound_level], dtype=torch.float64)/100.
    ...
    dynamics_model = utils.setup_relu((3, 6, 6, 2),
                                      params=None,
                                      negative_slope=0.01,
                                      bias=True,
                                      dtype=torch.float64)
    ...
    # print("check S:",S,S/np.linalg.norm(S,axis=1)[:, np.newaxis])
    # R = torch.from_numpy(S/np.linalg.norm(S,axis=1)[:, np.newaxis]) + 0.01 * torch.eye(4, dtype=dtype)
    R = torch.from_numpy(S) + 0.01 * torch.eye(4, dtype=dtype)
    ...
    forward_system = relu_system.ReLUSecondOrderResidueSystemGivenEquilibrium(
        torch.float64, x_lo, x_up, u_lo, u_up, dynamics_relu, q_equilibrium,
        u_equilibrium, dt,
    ...
    if args.train_cost_approximator:
        ...
        lyapunov_relu[0].device = device
        lyapunov_relu[2].device = device
    ...
    dut.lyapunov_derivative_convergence_tol = 3*1E-5
    ...
    dut.save_network_path = dir_path + "/data/monotonic_roa_0913/monotonic_bound" + str(bound_level) + "_"
    if args.train_adversarial:
        ...
        for i in range(1,8):
            dut.lyapunov_upper = 0.01*i#1.#None
        ...
        ## recursive training
        num_of_trials = 20#10
        sub_num_of_trials = 3#5
        dut.save_network_path = dir_path + "/data/monotonic_roa_0913/monotonic_bound" + str(bound_level+1) + "_"
        for m in range(2,2+num_of_trials):
            for j in range(sub_num_of_trials):
                ...
                if j>0:
                    for i in range(1,6):
                        dut.lyapunov_upper = 0.014*i#1.#None
                        print("ROA value: ",dut.lyapunov_upper)
                        dut.train(torch.empty((0, 4), dtype=torch.float64))
                    
                if j<sub_num_of_trials-1:
                    dut.max_iterations = 40#40
                    dut.lyapunov_upper = 0.07
                    dut.linf_expand_mip_cost_weight = 0.01#0.001
                    ...
                    dut.train(torch.empty((0, 4), dtype=torch.float64))
            dut.save_network_path = dir_path + "/data/monotonic_roa_0913/monotonic_bound" + str(bound_level+m) + "_"
            print("save_network_path: ", dut.save_network_path)
            print("--- %s seconds ---" % (time.time() - start_time))
        
        # dut.lyapunov_upper = 1.
