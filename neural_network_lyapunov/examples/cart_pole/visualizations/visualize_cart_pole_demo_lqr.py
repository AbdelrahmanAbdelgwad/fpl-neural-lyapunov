...
x_lo = torch.tensor([-1*bound_level_tmp,-np.pi/6*bound_level_tmp,
                        -1.5*bound_level_tmp,-1*bound_level_tmp], dtype=torch.float64)/60.
x_up = torch.tensor([1*bound_level_tmp,np.pi/6*bound_level_tmp,
                        1.5*bound_level_tmp,1*bound_level_tmp], dtype=torch.float64)/60.# x_des = np.array([np.pi, 0])
...
# load_lyapunov_relu = dir_path + "data/preprocess/lqr_lyapunov.pt"
# load_controller_relu = dir_path + "data/preprocess/lqr_controller.pt"
# load_R = dir_path + "data/preprocess/lqr_R.pt"
...
lyapunov_relu = torch.load(load_lyapunov_relu, map_location=torch.device('cpu'))
controller_relu = torch.load(load_controller_relu, map_location=torch.device('cpu'))
...
def plot_loss_historySeq(dir_path,boundList):
    ...
    for bound_level in boundList:
        ...
        # loss_history_dict['loss_history'] = [x.item() for x in loss_history_dict['loss_history']]
            ...
            print("bound level:",bound_level,", end loss:",loss_history_dict['loss_history'][-1].detach().numpy())
...
plot_loss_historySeq(dir_path+load_lossSeq_prefix,[*range(1, 50, 1)])
...
#%%
lyapunov_relu_init = utils.setup_relu((4, 8, 8, 6, 1),
                                    params=None,
                                    negative_slope=0.1,
                                    bias=True,
                                    dtype=torch.float64)
...
def plot_lyapunov_contour(x_lo,x_up,x_des,monotonic_NN=None,num_samples=[80,80],V_lambda=0.8,add_l1_state=False,R=None,display_v=False,roa_level=None,mode=None,plot_bLast=False,contour_idx=[2,3]):
    x_lo_np = x_lo.detach().numpy()
    x_up_np = x_up.detach().numpy()
    (pos_mesh, vel_mesh) = np.meshgrid(np.linspace(x_lo_np[contour_idx[0]], x_up_np[contour_idx[0]], num_samples[0]),
                            np.linspace(x_lo_np[contour_idx[1]], x_up_np[contour_idx[1]], num_samples[1]))
    zero_mesh = np.ones_like(pos_mesh)*0.01
    x_samples = torch.tensor(np.vstack(( np.reshape(zero_mesh, -1), np.reshape(zero_mesh, -1),
    ...
    X = x_samples[:,contour_idx[0]].reshape(num_samples[0], num_samples[1])
    Y =  x_samples[:,contour_idx[1]].reshape(num_samples[0], num_samples[1])
