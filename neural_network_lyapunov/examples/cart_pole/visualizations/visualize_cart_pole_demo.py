...
x_lo = torch.tensor([-1*bound_level,-np.pi/6*bound_level,
                        -1.*bound_level,-1*bound_level], dtype=torch.float64)/60.
x_up = torch.tensor([1*bound_level,np.pi/6*bound_level,
                        1.*bound_level,1*bound_level], dtype=torch.float64)/60.# x_des = np.array([np.pi, 0])
...
def plot_traj(lyapunov_hybrid_system,lyap_func,x_des,x_lo,x_up,N_x=6,dt=0.01,display_violation_states=True):
    ...
    for i in range(N_x):
        x=torch.tensor(xList[:,i],dtype=torch.float64)/60
        x_nextList = [x]
...
def plot_lqr_traj(lyapunov_hybrid_system,lyap_func,x_des,x_lo,x_up,N_x=6,dt=0.01,display_violation_states=True):
    ...
    for i in range(N_x):
        x=torch.tensor(xList[:,i],dtype=torch.float64)/30.
        x_nextList = [x]
        x_next_for_v = x_nextList[-1]
        # x_next_for_v = torch.cat(
        #         (lyapunov_hybrid_system.system.step_forward(x_nextList[-1])[0:2],
